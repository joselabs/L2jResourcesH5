...
public class RenameNPC extends Quest
{
	...
	private final static String RENAME_NPC_FEE = "57,250000";
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		String htmltext = "" + ServerStorage.getInstance().getString(lang, "RenameNPC.NEW_NAME") + "<br1><edit var=\"newname\" width=70 height=10>";
		...
		if (eventSplit[0].equalsIgnoreCase("rename"))
		{
			...
			{
				htmltext = "" + ServerStorage.getInstance().getString(lang, "RenameNPC.ENTER_NAME") + "";
			}
			else if (player.getLevel() < 40)
			{
				htmltext = "" + ServerStorage.getInstance().getString(lang, "RenameNPC.MIN_LVL") + " 40";
			}
			else if (validItemFee(st))
			{
				htmltext = "" + ServerStorage.getInstance().getString(lang, "RenameNPC.NO_ITEM") + "";
			}
			else if ((eventSplit[1].length() < 1) || (eventSplit[1].length() > 16))
			{
				htmltext = "" + ServerStorage.getInstance().getString(lang, "RenameNPC.MAX_LETTER") + "";
			}
			else if (!Util.isAlphaNumeric(eventSplit[1]))
			{
				htmltext = "" + ServerStorage.getInstance().getString(lang, "RenameNPC.LETTER_AND_DIGIT") + "";
			}
			else if (CharNameHolder.getInstance().doesCharNameExist(eventSplit[1]))
			{
				htmltext = "" + ServerStorage.getInstance().getString(lang, "RenameNPC.NAME_ALREADY_USE") + "";
			}
			else
			{
				try
				{
					...
					htmltext = "" + ServerStorage.getInstance().getString(lang, "RenameNPC.RENAME_SUH") + "";
					player.broadcastUserInfo(true);

					final String itemFeeSplit[] = RENAME_NPC_FEE.split("\\;");
					for (final String anItemFeeSplit : itemFeeSplit)
					{
						final String item[] = anItemFeeSplit.split("\\,");
	...
	public String onFirstTalk(Npc npc, Player player)
	{
		...
		htmltext = page(player, "" + ServerStorage.getInstance().getString(lang, "RenameNPC.NEW_NAME") + "<br1><edit var=\"newname\" width=70 height=10>", 0);
		return htmltext;
	}

	public String page(Player player, String msg, int t)
	{
		final String lang = player.getLang();
		String htmltext = "";
		htmltext += htmlPage(player, "Title");
		htmltext += "" + ServerStorage.getInstance().getString(lang, "RenameNPC.WELCOME") + "<br>" + "" + ServerStorage.getInstance().getString(lang, "RenameNPC.ENTER_NAME_NOW") + "<br1>";
		final String itemFeeSplit[] = RENAME_NPC_FEE.split("\\;");
		for (final String anItemFeeSplit : itemFeeSplit)
		{
			final String item[] = anItemFeeSplit.split("\\,");
			htmltext += "<font color=\"LEVEL\">" + item[1] + " " + player.getItemName(ItemsParser.getInstance().getTemplate(Integer.parseInt(item[0]))) + "</font><br1>";
		}
		if (t == 0)
		{
			htmltext += "<br><font color=\"339966\">" + msg + "</font>";
			htmltext += "<br><center>" + button("" + ServerStorage.getInstance().getString(lang, "RenameNPC.RENAME") + "", "rename $newname", 100, 23) + "</center>";
		}
		else
		{
			htmltext += "<br><font color=\"FF0000\">" + msg + "</font>";
			htmltext += "<br><center>" + button("" + ServerStorage.getInstance().getString(lang, "RenameNPC.BACK") + "", "" + ServerStorage.getInstance().getString(lang, "RenameNPC.START") + "", 70, 23) + "</center>";
		}
	...
	public String htmlPage(Player player, String op)
	{
		...
		if (op.equals("Title"))
		{
			texto += "<html><body><title>" + ServerStorage.getInstance().getString(lang, "RenameNPC.RENAME_MANAGER") + "</title><center><br>" + "<b><font color=ffcc00>" + ServerStorage.getInstance().getString(lang, "RenameNPC.INFO") + "</font></b>" + "<br><img src=\"L2UI_CH3.herotower_deco\" width=\"256\" height=\"32\"><br></center>";
		}
		...
		{
			texto = "" + ServerStorage.getInstance().getString(lang, "RenameNPC.NOT_FOUND") + "";
		}
		return texto;
	}
	...
	{
		new RenameNPC(-1, "RenameNPC", "services");
	}
}
