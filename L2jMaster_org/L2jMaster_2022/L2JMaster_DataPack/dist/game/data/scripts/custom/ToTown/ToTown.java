...
public class ToTown implements IVoicedCommandHandler
{
	...
	public boolean useVoicedCommand(String command, L2PcInstance activeChar, String target)
	{
		...
		else if ((Config.TELEPORT_TO_TOWN_COMMNAND) && (!activeChar.inObserverMode() && !activeChar.isInOlympiadMode() && !activeChar.isInDuel() && !activeChar.isInCombat() && !activeChar.isDead() && !activeChar.isJailed()))
		{
			...
			else if (command.startsWith("darkelf"))
			{
				activeChar.teleToLocation(11620, 16780, -4662);
			}
			else if (command.startsWith("elf"))
			{
				activeChar.teleToLocation(47050, 50767, -2996);
			}
			else if (command.startsWith("talking"))
			{
				activeChar.teleToLocation(-83644, 243702, -3729);
			}
			...
			else if (command.startsWith("hunters"))
			{
			...
			{
				activeChar.teleToLocation(113790, 15224, 9570);
			}
			...
			{
				activeChar.teleToLocation(52241, 218775, -3219);
			}
			else if (command.startsWith("orfen"))
			{
				activeChar.teleToLocation(47555, 17178, -4756);
			}
			else if (command.startsWith("core"))
			{
				activeChar.teleToLocation(17720, 118681, -6832);
			}
			else if (command.startsWith("antharas"))
			{
				activeChar.teleToLocation(154533, 121084, -3808);
			}
			else if (command.startsWith("lord"))
			{
				activeChar.teleToLocation(152909, 110570, -5525);
			}
			else if (command.startsWith("beast"))
			{
				activeChar.teleToLocation(147715, 117791, -3716);
			}
			else if (command.startsWith("behemoth"))
			{
				activeChar.teleToLocation(145937, 120790, -3917);
			}
			else if (command.startsWith("tezza"))
			{
				activeChar.teleToLocation(181370, -80813, -2732);
			}
			else if (command.startsWith("freya"))
			{
				activeChar.teleToLocation(103141, -124381, -2771);
			}
			else if (command.startsWith("beleth"))
			{
				activeChar.teleToLocation(16345, 209137, -9357);
			}
			else if (command.startsWith("darion"))
			{
				activeChar.teleToLocation(20886, 244088, 11088);
			}
			else
			{
