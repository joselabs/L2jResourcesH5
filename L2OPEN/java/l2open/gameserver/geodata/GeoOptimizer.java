...
{
	private static Logger log = Logger.getLogger(GeoOptimizer.class.getName());
	...
	public static class GeoBlocksMatchFinder extends RunnableImpl
	{
		...
		public GeoBlocksMatchFinder(int _geoX, int _geoY, int _maxScanRegions)
		{
			...
			geoX = _geoX;
			geoY = _geoY;
			rx = geoX + ConfigValue.GeoFirstX;
			ry = geoY + ConfigValue.GeoFirstY;
		...
		{
			return new File(ConfigValue.DatapackRoot, fileName).exists();
		}
		...
		private void saveToFile(BlockLink[] links)
		{
			...
			try
			{
				File f = new File(ConfigValue.DatapackRoot, fileName);
				if(f.exists())
					f.delete();
	...
	public static class CheckSumLoader extends RunnableImpl
	{
		...
		public CheckSumLoader(int _geoX, int _geoY, byte[][][] _region)
		{
			...
			geoY = _geoY;
			rx = geoX + ConfigValue.GeoFirstX;
			ry = _geoY + ConfigValue.GeoFirstY;
			region = _region;
		...
		{
			File GeoCrc = new File(ConfigValue.DatapackRoot, fileName);
			if(!GeoCrc.exists())
				return false;
			try
			{
