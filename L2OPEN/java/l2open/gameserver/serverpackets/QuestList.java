...
public class QuestList extends L2GameServerPacket
{
	...
	private byte[] _completedQuestsMask = new byte[128];
	...
	public QuestList(L2Player player)
	{
		...
		for(QuestState quest : player.getAllQuestsStates())
			if(quest != null)
			{
				int questId = quest.getQuest().getQuestIntId();
				if((questId < 999 || questId > 10000) && questId != 255)
				{
					if(quest.isStarted())
						questlist.add(new int[] { questId, quest.getRawInt("cond") }); // stage of quest progress
					else if(quest.isCompleted())
						_completedQuestsMask[(questId % 10000) / 8] |= 1 << (questId % 8);
				}
			}
	}
