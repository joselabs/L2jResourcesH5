...
public final class L2FestivalGuideInstance extends L2NpcInstance
{
		...
		else if(command.startsWith("Festival"))
		{
			...
			switch(val)
			{
				...
				case 2: // Festival 2 xxxx
					...
					{
						player.sendMessage(new CustomMessage("l2open.gameserver.model.instances.L2FestivalGuideInstance.InProgress", player));
						return;
					}

					// Check if the player is in a formed party already.
					if(playerParty == null || playerParty.getMemberCount() < ConfigValue.FestivalMinPartySize)
					{
					...
					long stonesNeeded = (long) Math.floor(SevenSignsFestival.getStoneCount(_festivalType, stoneType) * ConfigValue.FestivalRatePrice);
					...
					{
						player.sendMessage(new CustomMessage("l2open.gameserver.model.instances.L2FestivalGuideInstance.NotEnoughSSType", player));
						return;
					}
				...
				case 9: // Leave the Festival
					...
					else if(playerParty.getMemberCount() > ConfigValue.FestivalMinPartySize)
	...
	private void showChatWindow(L2Player player, int val, String suffix, boolean isDescription)
	{
		...
		html.replace("%min%", String.valueOf(ConfigValue.FestivalMinPartySize));
		// If the stats or bonus table is required, construct them.
		if(val == 1)
		{
			html.replace("%price1%", String.valueOf((long) Math.floor(SevenSignsFestival.getStoneCount(_festivalType, 6362) * ConfigValue.FestivalRatePrice)));
			html.replace("%price2%", String.valueOf((long) Math.floor(SevenSignsFestival.getStoneCount(_festivalType, 6361) * ConfigValue.FestivalRatePrice)));
			html.replace("%price3%", String.valueOf((long) Math.floor(SevenSignsFestival.getStoneCount(_festivalType, 6360) * ConfigValue.FestivalRatePrice)));
		}
		if(val == 5)
