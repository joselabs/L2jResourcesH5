package l2open.gameserver.model.entity;
...
import l2open.gameserver.model.AutoChatHandler;
import l2open.gameserver.model.AutoSpawnHandler;
import l2open.gameserver.model.AutoSpawnHandler.AutoSpawnInstance;
import l2open.gameserver.model.L2ObjectsStorage;
import l2open.gameserver.model.L2Player;
import l2open.gameserver.model.entity.SevenSignsFestival.SevenSignsFestival;
import l2open.gameserver.serverpackets.SSQInfo;
...
public class SevenSigns
{
	...
	public static final long MAXIMUM_PLAYER_CONTRIB = ConfigValue.MaxPlayerContribution;
	...
	public SevenSigns()
	{
		_signsPlayerData = new FastMap<Integer, StatsSet>().setShared(true);
		_signsSealOwners = new FastMap<Integer, Integer>().setShared(true);
		_signsDuskSealTotals = new FastMap<Integer, Integer>().setShared(true);
		_signsDawnSealTotals = new FastMap<Integer, Integer>().setShared(true);
		...
		if(ConfigValue.SSAnnouncePeriod > 0)
			ThreadPoolManager.getInstance().schedule(new SevenSignsAnnounce(), ConfigValue.SSAnnouncePeriod * 1000L * 60);
	}
	...
	public void spawnSevenSignsNPC()
	{
		...
		if(isSealValidationPeriod() || isCompResultsPeriod())
		{
			...
			{
				if(!ConfigValue.AnnounceMammonSpawn)
	...
	private static void processStatement(FiltredPreparedStatement statement, StatsSet sevenDat) throws SQLException
	{
	...
	public void sendCurrentPeriodMsg(L2Player player)
	{
		...
		{
			case PERIOD_COMP_RECRUITING:
				// Семь Печатей: Идет подготовка к новому кругу.
				player.sendPacket(Msg.SEVEN_SIGNS_PREPARATIONS_HAVE_BEGUN_FOR_THE_NEXT_QUEST_EVENT);
				return;
			case PERIOD_COMPETITION:
				// Семь Печатей: Ивент начался. Поговорите со Жрецом Рассвета или Жрицей Заката.
				...
				return;
			case PERIOD_COMP_RESULTS:
				// Семь Печатей: Ивент завершен. Собираются данные статистики.
				player.sendPacket(Msg.SEVEN_SIGNS_QUEST_EVENT_HAS_ENDED_RESULTS_ARE_BEING_TALLIED);
				return;
			case PERIOD_SEAL_VALIDATION:
				// Семь Печатей: Период действия Печати. Следующий ивент будет проведен в следующий понедельник.
	...
	public class SevenSignsAnnounce extends l2open.common.RunnableImpl
	{
		public void runImpl()
		{
			for(L2Player player : L2ObjectsStorage.getPlayers())
	 ...
	 */
	public class SevenSignsPeriodChange extends l2open.common.RunnableImpl
	{
		public void runImpl()
		{
			...
			switch(periodEnded)
			{
				...
				case PERIOD_COMP_RECRUITING: // Initialization
					// Семь Печатей: Квестовый ивент начался. Вы можете участвовать в нем, пообщавшись со Жрецом Рассвета или Жрицей Заката.
				...
				case PERIOD_COMPETITION: // Results Calculation
					// Семь Печатей: Квестовый ивент завершен. Следующий пройдет через неделю.
				...
				case PERIOD_COMP_RESULTS: // Seal Validation
					...
					// Семь Печатей: Печать активирована.
				...
				case PERIOD_SEAL_VALIDATION: // Reset for New Cycle
					...
					// Семь Печатей: Время действия Печати закончилось.
			...
			try
			{
				...
				setCalendarForNextPeriodChange();
				_log.info("SevenSignsPeriodChange: time to next change=" + Util.formatTime(getMilliToPeriodChange() / 1000));
