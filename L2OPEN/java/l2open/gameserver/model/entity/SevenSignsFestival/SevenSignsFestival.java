package l2open.gameserver.model.entity.SevenSignsFestival;
...
public class SevenSignsFestival
{
	...
	public static final short FESTIVAL_BLOOD_OFFERING = 5901;
	public static final short FESTIVAL_OFFERING_VALUE = 1;
	...
	public SevenSignsFestival()
	{
		...
		_dawnFestivalScores = new FastMap<Integer, Long>().setShared(true);
		_duskFestivalScores = new FastMap<Integer, Long>().setShared(true);
		_festivalData = new FastMap<Integer, Map<Integer, StatsSet>>().setShared(true);
		restoreFestivalData();
	}
	...
	public static String implodeString(List<?> strArray)
	{
		...
		for(int i = 0; i < strArray.size();)
		{
			...
			if(o instanceof L2Player)
				sb.append(((L2Player) o).getName());
			else
				sb.append(o);
			if(++i < strArray.size())
				sb.append(",");
		...
		try
		{
			...
			for(Map<Integer, StatsSet> currCycleData : _festivalData.values())
				for(StatsSet festivalDat : currCycleData.values())
				{
					...
					FiltredPreparedStatement statement2 = con.prepareStatement("INSERT INTO seven_signs_festival (festivalId, cabal, cycle, date, score, members, names) VALUES (?,?,?,?,?,?,?)");
	...
	{
		L2Player player = L2ObjectsStorage.getPlayer(Integer.parseInt(playerId));
		if(player != null)
		{
		...
		else
		{
			...
			{
				_log.warning("could not get clan name of " + playerId + ": " + e);
				e.printStackTrace();
			}
			finally
			{
	...
	public void resetFestivalData(boolean updateSettings)
	{
		...
		for(L2Player onlinePlayer : L2ObjectsStorage.getPlayers())
	 ...
	 */
	public boolean setFinalScore(L2Party party, int oracle, int festivalId, long offeringScore)
	{
		List<Integer> partyMemberIds = party.getPartyMembersObjIds();
		List<L2Player> partyMembers = party.getPartyMembers();
