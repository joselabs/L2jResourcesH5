...
public final class L2NpcTemplate extends L2CharTemplate
{
	...
	public L2NpcTemplate(StatsSet set)
	{
		...
		// TODO: Zkontrolovat jestli jdou spravně expy
		baseVitalityDivider = (level > 0) && (getRewardExp() > 0) ? (baseHpMax * 9 * level * level) / (100 * getRewardExp()) : 0;
		// baseVitalityDivider = (getLevel() > 0) && (getRewardExp() > 0) ? (getBaseHpMax() * 9 * getLevel() * getLevel()) / (100 * getRewardExp()) : 0;
	...
	/*
	 * public void addVulnerability(Stats id, double vuln) { if (_vulnerabilities == null) _vulnerabilities = new FastMap<Stats, Double>(); _vulnerabilities.put(id, new Double(vuln)); } public double getVulnerability(Stats id) { if (_vulnerabilities == null || _vulnerabilities.get(id) == null)
	 * return 1; return _vulnerabilities.get(id); }
	 */
	...
	/*
	 * public double removeVulnerability(Stats id) { return _vulnerabilities.remove(id); }
	 */
	...
	/*
	 * public void addQuestEvent(QuestEventType EventType, Quest q) { if (!_questEvents.containsKey(EventType)) { List<Quest> quests = new ArrayList<>(); quests.add(q); _questEvents.put(EventType, quests); } else { List<Quest> quests = _questEvents.get(EventType); if
	 * (!EventType.isMultipleRegistrationAllowed() && !quests.isEmpty()) { _log.warning("Quest event not allowed in multiple quests.  Skipped addition of Event Type \"" + EventType + "\" for NPC \"" + name + "\" and quest \"" + q.getName() + "\"."); } else { quests.add(q); } } }
	 */
	...
	/*
	 * public Map<QuestEventType, List<Quest>> getEventQuests() { return _questEvents; }
	 */
	
	/*
	 * public List<Quest> getEventQuests(QuestEventType EventType) { return _questEvents.get(EventType); }
	 */
