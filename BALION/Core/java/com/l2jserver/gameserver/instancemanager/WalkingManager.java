	...
	private class WalkInfo
	{
		...
		protected void calculateNextNode(L2Npc npc)
		{
			...
			if (getRoute().getRepeatType() == REPEAT_RANDOM)
			{
				...
				npc.sendMessage("Route: " + getRoute().getName() + ", next random node is " + _currentNode);
			}
			else
			{
				if (_forward)
				...
				if (_currentNode == getRoute().getNodesCount()) // Last node arrived
				{
					...
					npc.sendMessage("Route: " + getRoute().getName() + ", last node arrived");
	...
	protected void parseDocument()
	{
		...
		for (Node d = n.getFirstChild(); d != null; d = d.getNextSibling())
		{
			if (d.getNodeName().equals("route"))
			{
				...
				byte repeatType;
				if (repeatStyle.equals("back"))
				{
					repeatType = REPEAT_GO_BACK;
				}
				else if (repeatStyle.equals("cycle"))
				{
					repeatType = REPEAT_GO_FIRST;
				}
				else if (repeatStyle.equals("conveyor"))
				{
					repeatType = REPEAT_TELE_FIRST;
				}
				else if (repeatStyle.equals("random"))
				{
				...
				for (Node r = d.getFirstChild(); r != null; r = r.getNextSibling())
				{
					if (r.getNodeName().equals("point"))
					{
						...
						list.add(new L2NpcWalkerNode(0, x, y, z, delay, parseBoolean(attrs, "run")));
					}
	...
	public void onArrived(final L2Npc npc)
	{
		if (_activeRoutes.containsKey(npc.getObjectId()))
		{
			...
			if ((walk._currentNode >= 0) && (walk._currentNode < walk.getRoute().getNodesCount()))
			{
				...
				{
					npc.sendMessage("Route: " + walk.getRoute().getName() + ", arrived to node " + walk._currentNode);
					npc.sendMessage("Done in " + ((System.currentTimeMillis() - walk._lastActionTime) / 1000) + " s.");
					walk.calculateNextNode(npc);
