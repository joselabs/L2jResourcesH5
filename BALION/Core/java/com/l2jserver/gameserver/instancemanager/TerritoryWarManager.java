...
public class TerritoryWarManager implements Siegable
{
	...
	protected final FastList<Integer> _disguisedPlayers;
	...
	protected final Calendar _startTWDate = Calendar.getInstance();
	...
	/**
	 * Return true if the clan is registered<BR>
	 * <BR>
	...
	/**
	 * Return true if the player is registered<BR>
	 * <BR>
	...
	public L2Npc spawnNPC(int npcId, Location loc)
	{
		...
		if (template != null)
		{
			...
			try
			{
				...
				spawnDat.setXLocation(loc.getX());
				spawnDat.setYLocation(loc.getY());
				spawnDat.setZLocation(loc.getZ());
	...
	{
		try (InputStream is = new FileInputStream(new File(Config.TERITORRIES_WARS)))
		{
