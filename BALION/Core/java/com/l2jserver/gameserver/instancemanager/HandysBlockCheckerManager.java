...
public final class HandysBlockCheckerManager
{
	...
	public boolean addPlayerToArena(L2PcInstance player, int arenaId)
	{
		...
		synchronized (holder)
		{
			...
			if (TvTRoundEvent.isPlayerParticipant(player.getObjectId()) || player.isInOlympiadMode())
			{
				player.sendMessage("Couldnt register you due other event participation");
				return false;
			}
			...
			/*
			 * if(UnderGroundColiseum.getInstance().isRegisteredPlayer(player)) { UngerGroundColiseum.getInstance().removeParticipant(player); player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.COLISEUM_OLYMPIAD_KRATEIS_APPLICANTS_CANNOT_PARTICIPATE)); }
			 * if(KrateiCubeManager.getInstance().isRegisteredPlayer(player)) { KrateiCubeManager.getInstance().removeParticipant(player); player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.COLISEUM_OLYMPIAD_KRATEIS_APPLICANTS_CANNOT_PARTICIPATE)); }
			 */
