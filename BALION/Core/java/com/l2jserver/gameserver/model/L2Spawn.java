...
public class L2Spawn
{
	...
	public L2Spawn(L2NpcTemplate mobTemplate) throws SecurityException, ClassNotFoundException, NoSuchMethodException
	{
		...
		{
			int.class,
			Class.forName("com.l2jserver.gameserver.templates.chars.L2NpcTemplate")
		};
	...
	private final Location _location = new Location(0, 0, 0, 0, 0);
	 ...
	 * <BR>
	 */
	public void setZLocation(int locz)
	{
		_locZ = locz;
	}
	...
	public L2Npc doSpawn(boolean isSummonSpawn)
	{
		...
		try
		{
			...
			if (_template.type.equalsIgnoreCase("L2Pet") || _template.type.equalsIgnoreCase("L2Decoy") || _template.type.equalsIgnoreCase("L2Trap") || _template.type.equalsIgnoreCase("L2EffectPoint"))
			{
				_currentCount++;
	...
	private L2Npc initializeNpcInstance(L2Npc mob)
	{
		...
		if ((getLocx() == 0) && (getLocy() == 0))
		{
			...
			int p[] = Territory.getInstance().getRandomPoint(getLocationId());
		...
		if (Config.CHAMPION_ENABLE)
		{
			...
			if (mob.isMonster() && !getTemplate().isUndead() && !mob.isRaid() && !mob.isRaidMinion() && (mob.getLevel() >= ChampionData.getInstance().getMinLv(false)) && (mob.getLevel() <= ChampionData.getInstance().getMaxLv(false)) && (ChampionData.getInstance().inInstanceEnabled() || (getInstanceId() == 0)))
			{
			...
			if (mob.isMonster() && !getTemplate().isUndead() && !mob.isRaid() && !mob.isRaidMinion() && (mob.getLevel() >= ChampionData.getInstance().getMinLv(true)) && (mob.getLevel() <= ChampionData.getInstance().getMaxLv(true)) && (ChampionData.getInstance().inInstanceEnabled() || (getInstanceId() == 0)))
			{
