...
public abstract class L2Skill implements IChanceSkillTrigger
{
					...
					if (player.getClan() != null)
					{
						...
						// synchronized (activeChar.getKnownList().getKnownObjects())
						{
							for (L2PcInstance obj : objs)
							{
								...
								if (!TvTRoundEvent.checkForTvTRoundSkill(player, obj, this))
								{
									continue;
								}
	...
	public static final boolean checkForAreaOffensiveSkills(L2Character caster, L2Character target, L2Skill skill, boolean sourceInArena)
	{
		...
		if (player != null)
		{
			if (targetPlayer != null)
			{
				// Add by CTF Event - check if allow interference is allowed if player is not on event but target is on event
				if ((CTF._started && !Config.CTF_ALLOW_INTERFERENCE) && !player.isGM())
				{
					if ((targetPlayer._inEventCTF && !player._inEventCTF) || (!targetPlayer._inEventCTF && player._inEventCTF))
					{
						return false;
					}
				}
