...
public class TvTEvent
{
	...
	public static boolean startParticipation()
	{
		...
		try
		{
			...
			_npcSpawn.setXLocation(Config.TVT_EVENT_PARTICIPATION_NPC_COORDINATES[0]);
			_npcSpawn.setYLocation(Config.TVT_EVENT_PARTICIPATION_NPC_COORDINATES[1]);
			_npcSpawn.setZLocation(Config.TVT_EVENT_PARTICIPATION_NPC_COORDINATES[2]);
	...
	public static synchronized void onBypass(String command, L2PcInstance playerInstance, boolean voiceCommand)
	{
		...
		if (!isParticipating())
		{
			if (voiceCommand)
			{
				playerInstance.sendMessage("You can't use this command now. Use this command when will be time for TVT Event registration");
			}
