...
public class TvTRoundEvent
{
	...
	public static void closeAnteroomDoors()
	{
		...
		ThreadPoolManager.getInstance().scheduleGeneral(() -> closeDoors(Config.TVT_ROUND_ANTEROOM_DOORS_IDS_TO_OPEN_CLOSE), TvTRoundWaitCloseAnteroomDoors * 1000);
	}
		...
		for (L2PcInstance playerInstance : team.getParticipatedPlayers().values())
		{
			...
			for (int[] reward : Config.TVT_ROUND_EVENT_REWARDS)
			{
				...
				if (reward[0] == -300)
				{
					...
					playerInstance.sendMessage("[TvTEvent]: You have received " + reward[1] + " Fame.");
					continue;
				}
				if (reward[0] == -100)
				{
					playerInstance.addPcBangScore(reward[1]);
					playerInstance.sendMessage("[TvTEvent]: You have received " + reward[1] + " PcBang Points.");
					continue;
				}
	 ...
	 * @param playerInstance as L2PcInstance<br>
	 */
	public static synchronized void onBypass(String command, L2PcInstance playerInstance, boolean voiceCommand)
	{
		...
		if (!isParticipating())
		{
			if (voiceCommand)
			{
				playerInstance.sendMessage("You can't use this command now. Use this command when will be time for TVT Round Event registration");
			}
