			...
			if (spawn != null)
			{
				...
				for (SiegeSpawn spawn2 : commanders)
				{
					if (spawn2.getNpcId() == spawn.getNpcId())
					{
						...
						if (npcString != null)
						{
							instance.broadcastPacket(new CreatureSay(instance.getObjectId(), 1, instance.getName(), npcString));
						}
					}
				}
		...
		try
		{
			...
			for (SiegeSpawn _sp : FortSiegeManager.getInstance().getCommanderSpawnList(getFort().getFortId()))
			{
				...
				if (template1 != null)
				{
					...
					spawnDat.setXLocation(_sp.getLocation().getX());
					spawnDat.setYLocation(_sp.getLocation().getY());
					spawnDat.setZLocation(_sp.getLocation().getZ());
