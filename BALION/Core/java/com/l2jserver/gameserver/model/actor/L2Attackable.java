...
public class L2Attackable extends L2Npc
{
	...
	/**
	 * Not used. get condition to hate, actually isAggressive() is checked by monster and karma by guards in motheds that overwrite this one.
	 * @deprecated
	 */
	@Deprecated
	...
	public static class OnKillNotifyTask implements Runnable
	{
	...
	/**
	 * Distribute Exp and SP rewards to L2PcInstance (including Summon owner) that hit the L2Attackable and to their Party members. Actions: Get the L2PcInstance owner of the L2SummonInstance (if necessary) and L2Party in progress Calculate the Experience and SP rewards in function of the level
	...
	private RewardItem calculateRewardItem(L2PcInstance lastAttacker, L2DropData drop, int levelModifier, boolean isSweep)
	{
		...
		if (Config.CHAMPION_ENABLE && isChampion())
		{
			dropChance *= ChampionData.getInstance().getRewardMultipler(this);
		}
		...
		if (Config.CHAMPION_ENABLE)
		{
			...
			{
				itemCount *= ChampionData.getInstance().getAdenaMultipler(this);
			}
		}
	...
	private RewardItem calculateCategorizedRewardItem(L2PcInstance lastAttacker, L2DropCategory categoryDrops, int levelModifier)
	{
		...
		if (Config.CHAMPION_ENABLE && isChampion())
		{
			categoryDropChance *= ChampionData.getInstance().getRewardMultipler(this);
		}
		...
		if (Rnd.get(L2DropData.MAX_CHANCE) < categoryDropChance)
		{
			...
			/*
			 * if ((Config.PREMIUM_RATE_DROP_ITEMS_ID.get(drop.getItemId()) != 0) && (lastAttacker.getPremiumService() == 1)) { dropChance *= Config.PREMIUM_RATE_DROP_ITEMS_ID.get(drop.getItemId()); } else if (Config.RATE_DROP_ITEMS_ID.get(drop.getItemId()) != 0) { dropChance *=
			 * Config.RATE_DROP_ITEMS_ID.get(drop.getItemId()); } if (lastAttacker.getPremiumService() == 1) { dropChance *= isRaid() && !isRaidMinion() ? Config.PREMIUM_RATE_DROP_ITEMS_BY_RAID : Config.PREMIUM_RATE_DROP_ITEMS; } else { dropChance *= isRaid() && !isRaidMinion() ?
			 * Config.RATE_DROP_ITEMS_BY_RAID : Config.RATE_DROP_ITEMS; }
			 */
			...
			else if (lastAttacker.getPremiumService() == 0)
			{
	...
	public void doItemDrop(L2NpcTemplate npcTemplate, L2Character mainDamageDealer)
	{
		...
		if (Config.CHAMPION_ENABLE && isChampion() && ((Config.CHAMPION_REWARD_LOWER_LVL_ITEM_CHANCE > 0) || (Config.CHAMPION_REWARD_HIGHER_LVL_ITEM_CHANCE > 0)))
		{
			...
			RewardItem item = new RewardItem(ChampionData.getInstance().getRewardId(this), ++champqty);
	 ...
	 * automaticaly. Actions: <I>If an extra drop must be generated</I> Get an Item Identifier (random) from the DateDrop Item table of this Event Get the Item quantity dropped (random) Create this or these L2ItemInstance corresponding to this Item Identifier If the autoLoot mode is actif and if the
	 * L2Character that has killed the L2Attackable is a L2PcInstance, Give the item(s) to the L2PcInstance that has killed the L2Attackable If the autoLoot mode isn't actif or if the L2Character that has killed the L2Attackable is not a L2PcInstance, add this or these item(s) in the world as a
	...
	/**
	 * Adds an attacker that successfully absorbed the soul of this L2Attackable into the _absorbersList. Params: attacker - a valid L2PcInstance condition - an integer indicating the event when mob dies. This should be: = 0 - "the crystal scatters"; = 1 - "the crystal failed to absorb. nothing
	 * happens"; = 2 - "the crystal resonates because you got more than 1 crystal on you"; = 3 - "the crystal cannot absorb the soul because the mob level is too low"; = 4 - "the crystal successfuly absorbed the soul";
	 */
	...
	public void ai_attack(L2Attackable npc, L2PcInstance player)
	{
		npc.setRunning();
