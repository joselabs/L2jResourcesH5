...
import com.l2jserver.gameserver.model.Strings;
...
public final class L2PcInstance extends L2Playable
{
	...
	/** PvP Quake Reward System (By Kryspo) */
	...
	public int _recomHave; // how much I was recommended by others
	/** The number of recommendation that the L2PcInstance can give */
	public int _recomLeft; // how many recommendations I can give to others
	/** Recommendation Bonus task **/
	public ScheduledFuture<?> _recoBonusTask;
	public boolean _isRecoBonusActive = false;
	public int _recoBonusMode = 0;
	/** Recommendation task **/
	public ScheduledFuture<?> _recoGiveTask;
	/** Recommendation Two Hours bonus **/
	public boolean _recoTwoHoursGiven = false;
	...
	public static final Set<String> DEBUG_CORE_MESSAGE = new CopyOnWriteArraySet<>();
	...
	public void rewardSkills()
	{
		...
		for (Entry<Integer, Integer> entry : Config.STARTING_COMMON_SKILLS.entrySet())
		{
			addSkill(SkillTable.getInstance().getSkill(entry.getKey(), entry.getValue()));
		}
	...
	public boolean doDie(L2Character killer)
	{
		...
		else if (Config.TW_RESS_ON_DEATH && isInTownWarEvent())
		{
			reviveRequest(this, null, false);
		}
	...
	public void increasePvpKills(L2Character target)
	{
		...
		if ((target instanceof L2PcInstance) && AntiFeedManager.getInstance().check(this, target))
		{
			...
			{
				// Item Reward system
				addItem("Loot", Config.KRYSPO_PVP_REWARD_ITEM, Config.KRYSPO_PVP_REWARD_COUNT, this, true);
				sendMessage("You will be rewarded for pvp kill!");
			}
			if (Config.KRYSPO_COSTUM_MSG_ALLOWED)
			{
				sendMessage("Enjoy your PvP reward");
			}
			if (Config.QUAKEPVP_QUAKE_PVP_ENABLE)
			{
				QuakePvPSystem++;
				switch (QuakePvPSystem)
				{
					case 1:
						...
						sendPacket(_snd1);
						sendMessage("You will be rewarded for 1 PvP kill in Quake System.");
						break;
					case 2:
						addItem("Loot", Config.QUAKE_REWARD_2, Config.QUAKE_COUNT_2, this, true);
						PlaySound _snd2 = new PlaySound(1, "doublekill", 0, 0, 0, 0, 0);
						sendPacket(_snd2);
						sendMessage("You will be rewarded for 2 PvP kill in Quake System.");
						break;
					case 3:
						addItem("Loot", Config.QUAKE_REWARD_3, Config.QUAKE_COUNT_3, this, true);
						PlaySound _snd3 = new PlaySound(1, "triplekill", 0, 0, 0, 0, 0);
						sendPacket(_snd3);
						sendMessage("You will be rewarded for 3 PvP kill in Quake System.");
						break;
					case 5:
						addItem("Loot", Config.QUAKE_REWARD_5, Config.QUAKE_COUNT_5, this, true);
						PlaySound _snd5 = new PlaySound(1, "megakill", 0, 0, 0, 0, 0);
						sendPacket(_snd5);
						sendMessage("You will be rewarded for 5 PvP kill in Quake System.");
						break;
					case 10:
						addItem("Loot", Config.QUAKE_REWARD_10, Config.QUAKE_COUNT_10, this, true);
						PlaySound _snd10 = new PlaySound(1, "ultrakill", 0, 0, 0, 0, 0);
						sendPacket(_snd10);
						sendMessage("You will be rewarded for 10 PvP kill in Quake System.");
						Announcements.getInstance().announceToAll("Player " + target.getName() + " reach 10 PvP kills in without death.");
						break;
					case 15:
						addItem("Loot", Config.QUAKE_REWARD_15, Config.QUAKE_COUNT_15, this, true);
						PlaySound _snd15 = new PlaySound(1, "monsterkill", 0, 0, 0, 0, 0);
						sendPacket(_snd15);
						sendMessage("You will be rewarded for 15 PvP kill in Quake System.");
						Announcements.getInstance().announceToAll("Player " + target.getName() + " reach 15 PvP kills in without death.");
						break;
					case 25:
						addItem("Loot", Config.QUAKE_REWARD_25, Config.QUAKE_COUNT_25, this, true);
						PlaySound _snd25 = new PlaySound(1, "killingspree", 0, 0, 0, 0, 0);
						sendPacket(_snd25);
						sendMessage("You will be rewarded for 25 PvP kill in Quake System.");
						Announcements.getInstance().announceToAll("Player " + target.getName() + " reach 25 PvP kills in without death.");
						if (Config.QUAKEPVP_QUAKE_HERO_ENABLE)
						{
							sendMessage("You will be a Hero player till you restart the game");
							setHero(true);// Very unstable gameplay. if clan go to GB or Siege.. all players can be hero from this clan... Ultimate power :-D
							_log.info("Be careful ADMIN for this player: " + target.getName() + " he reach HERO status from Quake System."); // just for warning and info if player killing dualbox.. you will see many times hero status for this player
						}
					default:
				}
			}
	...
	public boolean mountPlayer(L2Summon pet)
	{
		if ((pet != null) && pet.isMountable() && !isMounted() && !isBetrayed())
		{
			...
			else if (_haveFlagCTF)
			{
				...
				sendPacket(ActionFailed.STATIC_PACKET);
				sendPacket(SystemMessage.getSystemMessage(SystemMessageId.YOU_CANNOT_BOARD_AN_AIRSHIP_WHILE_HOLDING_A_FLAG));
				return false;
			}
			else if (pet.isDead())
			{
	...
	private void storeCharBase()
	{
		...
		try
		{
			...
			statement.setInt(52, getPcBangPoints());
	...
	public boolean isAutoAttackable(L2Character attacker)
	{
		...
		// Check if the attacker is in TvT Round and TvT Round is started
		if (TvTRoundEvent.isStarted() && TvTRoundEvent.isPlayerParticipant(getObjectId()))
		{
			return true;
		}
	...
	private boolean checkUseMagicConditions(L2Skill skill, boolean forceUse, boolean dontMove)
	{
		...
		if (skill.getSkillType() == L2SkillType.INSTANT_JUMP)
		{
			...
			if (isInsideZone(L2Character.ZONE_PEACE) && !isInTownWarEvent())
			{
				if (!this.isInFunEvent() || !target.isInFunEvent())
				{
	...
	public void doRevive()
	{
		...
		if (_inEventCTF && CTF._started && Config.CTF_REVIVE_RECOVERY)
		{
			...
			if (Config.CTF_BUFFS_AFTER_DIE)
			{
				if (isMageClass())
				{
					if ((Config.CTF_EVENT_MAGE_BUFFS != null) && !Config.CTF_EVENT_MAGE_BUFFS.isEmpty())
					{
						for (int i : Config.CTF_EVENT_MAGE_BUFFS.keys())
						{
							final L2Skill skill = SkillTable.getInstance().getInfo(i, Config.CTF_EVENT_MAGE_BUFFS.get(i));
							if (skill != null)
							{
				...
				else
				{
					...
					{
						for (int i : Config.CTF_EVENT_FIGHTER_BUFFS.keys())
						{
							final L2Skill skill = SkillTable.getInstance().getInfo(i, Config.CTF_EVENT_FIGHTER_BUFFS.get(i));
							if (skill != null)
							{
								skill.getEffects(this, this);
							}
	...
	public void setPunishLevel(PunishLevel state, int delayInMinutes)
	{
		...
		switch (state)
		{
			...
			case JAIL: // Jail Player
			{
				...
				if (!TvTRoundEvent.isInactive() && TvTRoundEvent.isPlayerParticipant(getObjectId()))
				{
					TvTRoundEvent.removeParticipant(getObjectId());
				}
	...
	{
		if (((getKarma() > 0) || (Rnd.get(1, 100) <= Config.DEATH_PENALTY_CHANCE)) && !(killer instanceof L2PcInstance) && !(isGM()) && !(getCharmOfLuck() && killer.isRaid()) && !isPhoenixBlessed() && !isLucky() && !(TvTEvent.isStarted() && TvTEvent.isPlayerParticipant(getObjectId())) && !(TvTRoundEvent.isStarted() && TvTRoundEvent.isPlayerParticipant(getObjectId())) && !(isInsideZone(L2Character.ZONE_PVP) || isInsideZone(L2Character.ZONE_SIEGE)))
		{
	...
	public static boolean checkSummonerStatus(L2PcInstance summonerChar)
	{
		...
		if (!TvTRoundEvent.onEscapeUse(summonerChar.getObjectId()))
		{
			summonerChar.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.YOUR_TARGET_IS_IN_AN_AREA_WHICH_BLOCKS_SUMMONING));
			return false;
		}
	...
	/**
	 * Set the _creationTime of the L2PcInstance.<BR>
	 * <BR>
	 */
	...
	/**
	 * Return the _creationTime of the L2PcInstance.<BR>
	 * <BR>
	 */
	...
	/**
	 * Returns true if receiving item auction requests (last request was in 2 seconds before)
	 */
	...
	public void setVar(String name, String value)
	{
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("REPLACE INTO user_variables (obj_id, type, name, value, expire_time) VALUES (?,'user-var',?,?,-1)"))
		{
			statement.setInt(1, getObjectId());
	...
	public void unsetVar(String name)
	{
		...
		{
			try (Connection con = L2DatabaseFactory.getInstance().getConnection();
				PreparedStatement statement = con.prepareStatement("DELETE FROM `user_variables` WHERE `obj_id`=? AND `type`='user-var' AND `name`=? LIMIT 1"))
			{
				statement.setInt(1, getObjectId());
				statement.setString(2, name);
				statement.execute();
	...
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement offline = con.prepareStatement("SELECT * FROM user_variables WHERE obj_id = ?"))
		{
	...
	public void restoreVisualArmors()
	{
		Connection con = null;
		try
		{
	...
	// TODO: Icons
	public void removeVisualArmors()
	{
		Connection con = null;
		try
		{
			con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("DELETE FROM character_visual_armors WHERE charId=?");
			statement.setInt(1, getObjectId());
			statement.execute();
			statement.close();
		}
		...
		{
			_log.log(Level.WARNING, "Delete visual armors data where player:  " + getObjectId() + ". " + e.getMessage(), e);
		}
		finally
		{
			L2DatabaseFactory.close(con);
	...
	public final void setXYZInvisible(int[] location)
	{
		setXYZInvisible(location[0], location[1], location[2]);
	}
