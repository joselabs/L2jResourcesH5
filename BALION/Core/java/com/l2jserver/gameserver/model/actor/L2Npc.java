...
import com.l2jserver.gameserver.model.entity.MonsterRushTemplates;
...
public class L2Npc extends L2Character
{
	...
	public boolean doDie(L2Character killer)
	{
		...
		/** MonsterRush */
		if (getTemplate().npcId == MonsterRushTemplates.LORD)
		{
			deleteMe();
	...
	public int getExpReward(int isPremium)
	{
		if (Config.ENABLED_BOOST_EXP)
		{
			if (isPremium == 1)
			{
				return (int) (getTemplate().getRewardExp() * Config.INT_RATE_XP_OF_TODAY_PREMIUM);
			}
			else
			{
				return (int) (getTemplate().getRewardExp() * Config.INT_RATE_XP_OF_TODAY);
			}
		}
		if (isPremium == 1)
		{
	...
	public int getSpReward(int isPremium)
	{
		if (Config.ENABLED_BOOST_EXP)
		{
			if (isPremium == 1)
			{
				return (int) (getTemplate().getRewardSp() * Config.INT_RATE_SP_OF_TODAY_PREMIUM);
			}
			else
			{
				return (int) (getTemplate().getRewardSp() * Config.INT_RATE_SP_OF_TODAY);
			}
		}
		if (isPremium == 1)
		{
	...
	public void broadcastEvent(String eventName, int radius, L2Object reference)
	{
		for (L2Object obj : L2World.getInstance().getVisibleObjects(this, radius))
		{
			...
			if (obj.isMonster() && (((L2Npc) obj).getTemplate().getEventQuests(QuestEventType.ON_EVENT_RECEIVED) != null))
			{
				for (Quest quest : ((L2Npc) obj).getTemplate().getEventQuests(QuestEventType.ON_EVENT_RECEIVED))
				{
					quest.notifyEventReceived(eventName, this, (L2Npc) obj, reference);
	...
	public L2ItemInstance dropItem(L2PcInstance player, int itemId, long itemCount)
	{
		...
		for (int i = 0; i < itemCount; i++)
		{
			...
			{
				_log.info("Item doesn't exist so cannot be dropped. Item ID: " + itemId + " Quest: " + getName());
				return null;
			}
