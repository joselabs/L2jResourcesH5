...
public class OlympiadGameTeams extends AbstractOlympiadGame
{
	...
	protected final void cleanEffects()
	{
		...
		{
			par = _teamOne[i];
			if ((par.player != null) && !par.defaulted && !par.disconnected && (par.player.getOlympiadGameId() == _stadiumID))
			{
	...
	protected final void portPlayersBack()
	{
		...
		{
			par = _teamOne[i];
			if ((par.player != null) && !par.defaulted && !par.disconnected)
			{
	...
	protected void validateWinner(L2OlympiadStadiumZone stadium)
	{
		...
		try
		{
			...
			{
				par = _teamOne[i];
				if (!par.disconnected && (par.player != null) && !par.player.isDead())
				{
