...
public abstract class L2Object
{
			...
			if ((_instanceId > 0) && (oldI != null))
			{
				...
				if (oldI.isShowTimer())
				{
					...
					if (oldI.isTimerIncrease())
					{
						...
						sendPacket(new ExSendUIEvent(getActingPlayer(), hide_1, true, startTime, endTime, oldI.getTimerText()));
						
					}
					else
					{
						// sendPacket(new ExSendUIEvent(this, true, false, endTime - startTime, 0, oldI.getTimerText()));
						sendPacket(new ExSendUIEvent(getActingPlayer(), hide_0, false, endTime - startTime, 0, oldI.getTimerText()));
			...
			if (instanceId > 0)
			{
				...
				if (newI.isShowTimer())
				{
					...
					if (newI.isTimerIncrease())
					{
						...
						sendPacket(new ExSendUIEvent(getActingPlayer(), hide_0, true, startTime, endTime, oldI.getTimerText()));
					}
					else
					{
	 ...
	 */
	public final double calculateDistance(Location loc, boolean includeZAxis, boolean squared)
	{
	 ...
	 */
	public final double calculateDirectionTo(Location target)
	{
		int heading = Util.calculateHeadingFrom(this.getLocationXYZ(), target) - getHeading();
		if (heading < 0)
		{
