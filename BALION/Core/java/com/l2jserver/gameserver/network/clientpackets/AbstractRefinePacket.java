...
public abstract class AbstractRefinePacket extends L2GameClientPacket
{
	...
	protected static final boolean isValid(L2PcInstance player, L2ItemInstance item)
	{
		...
		if (item.isHeroItem() && !Config.ALT_ALLOW_AUGMENT_HERO_ITEMS)
		{
			return false;
		}
		if (item.isShadowItem() && !Config.ALT_ALLOW_AUGMENT_SHADOW_ITEMS)
		{
			return false;
		}
		if (item.isCommonItem() && !Config.ALT_ALLOW_AUGMENT_COMMON_ITEMS)
		{
			return false;
		}
		if (item.isEtcItem())
		...
		if (item.isTimeLimitedItem() && !Config.ALT_ALLOW_AUGMENT_LIMITED_TIME_ITEMS)
		{
			return false;
		}
