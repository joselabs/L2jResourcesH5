...
public class MultiSellChoose extends L2GameClientPacket
{
	...
	public void runImpl()
	{
		...
		// I think we need to just remove if player is GM or not Im really suck at if else....
		if (!list.isAllowBBS())
		{
			// Target is really == ? doesnt matter..ok
			if (((target == null) || !list.checkNpcObjectId(target.getObjectId()) || !target.canInteract(player)) && !player.isGM())
			{
				player.setMultiSell(null);
				return;
			}
