...
public class SevenSigns
{
	...
	/**
	 * Saves all Seven Signs player data. Should be called on period change and shutdown only. <BR>
	 * @param player
	...
	public int setPlayerInfo(int objectId, int chosenCabal, int chosenSeal)
	{
		...
		else
		{
			...
			try
			{
				...
				if (Config.DEBUG)
				{
					_log.info("SevenSigns: Inserted data in DB for char ID " + currPlayerData.getInt("charId") + " (" + currPlayerData.getString("cabal") + ")");
				}
			}
			catch (SQLException e)
			{
