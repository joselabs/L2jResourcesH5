	...
	private void thinkActive()
	{
		...
		else if ((npc.getSpawn() != null) && (Rnd.nextInt(RANDOM_WALK_RATE) == 0) && !npc.isNoRndWalk())
		{
			...
			if ((npc.getSpawn().getLocx() == 0) && (npc.getSpawn().getLocy() == 0))
			{
				...
				int p[] = Territory.getInstance().getRandomPoint(npc.getSpawn().getLocationId());
				x1 = p[0];
				y1 = p[1];
				z1 = p[2];
				...
				if ((Territory.getInstance().getProcMax(npc.getSpawn().getLocationId()) > 0) && !npc.isReturningToSpawnPoint())
				{
					return;
				}
			}
	...
	private void movementDisable()
	{
		...
		try
		{
			...
			if (_skillrender.hasSkill())
			{
				...
				if (_skillrender.hasAtkSkill() && (npc.isMovementDisabled() || (npc.getAiType() == AIType.MAGE) || (npc.getAiType() == AIType.HEALER)))
				{
					for (L2Skill sk : _skillrender._atkskills)
					{
						if ((sk.getMpConsume() >= npc.getCurrentMp()) || npc.isSkillDisabled(sk) || (((sk.getCastRange() + npc.getTemplate().collisionRadius + getAttackTarget().getTemplate().collisionRadius) <= dist2) && !canAura(sk)) || (sk.isMagic() && npc.isMuted()) || (!sk.isMagic() && npc.isPhysicalMuted()))
				...
				/*
				 * if(_skillrender.hasUniversalSkill()) { for(L2Skill sk:_skillrender._universalskills) { if(sk.getMpConsume()>=_actor.getCurrentMp() || _actor.isSkillDisabled(sk.getId()) ||(sk.getCastRange()+ _actor.getTemplate().collisionRadius + getAttackTarget().getTemplate().collisionRadius <=
