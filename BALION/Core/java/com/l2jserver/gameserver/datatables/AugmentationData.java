...
public class AugmentationData
{
	...
	{
		_log.info("Initializing Augmentation.");
		...
		_log.info("Augmentation: " + (_augStats[0].size() * 4) + " augmentation stats.");
		_log.info("Augmentation: " + (_augAccStats[0].size() * 4) + " accessory augmentation stats.");
		for (int i = 0; i < 10; i++)
		{
			_log.info("Augmentation: " + _blueSkills[i].size() + " blue, " + _purpleSkills[i].size() + " purple and " + _redSkills[i].size() + " red skills for lifeStoneLevel " + i);
		}
	}
	...
	private final void load()
	{
		...
		try
		{
			...
			if (badAugmantData != 0)
			{
				_log.info("Augmentation: " + badAugmantData + " bad skill(s) were skipped.");
			}
		}
		catch (Exception e)
		{
