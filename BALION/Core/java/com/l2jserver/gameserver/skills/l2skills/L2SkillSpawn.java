...
public class L2SkillSpawn extends L2Skill
{
	...
	public void useSkill(L2Character caster, L2Object[] targets)
	{
		...
		try
		{
			...
			if (_randomOffset)
			{
				spawn.setXLocation(caster.getX() + (Rnd.nextBoolean() ? Rnd.get(20, 50) : Rnd.get(-50, -20)));
				spawn.setYLocation(caster.getY() + (Rnd.nextBoolean() ? Rnd.get(20, 50) : Rnd.get(-50, -20)));
			}
			else
			{
				spawn.setXLocation(caster.getX());
				spawn.setYLocation(caster.getY());
			}
			spawn.setZLocation(caster.getZ() + 20);
