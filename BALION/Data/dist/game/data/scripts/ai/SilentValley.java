...
public class SilentValley extends L2AttackableAIScript
{
	...
	public SilentValley()
	{
		super(-1, SilentValley.class.getSimpleName(), "ai/group_template");
		AttackNpcs(MOBS);
		AttackNpcs(CHEST, GUARD1, GUARD2);
		addEventReceivedId(GUARD1, GUARD2);
		KillNpcs(MOBS);
		SeeCreatureNpcs(MOBS);
		SeeCreatureNpcs(GUARD1, GUARD2);
		SpawnNpcs(CHEST, GUARD2);
	}
	...
	public String onSeeCreature(L2Npc npc, L2Character creature, boolean isSummon)
	{
		if (creature.isPlayable())
		{
			...
			if ((npc.getNpcId() == GUARD1) || (npc.getNpcId() == GUARD2))
			{
				npc.setTarget(player);
				npc.doCast(BLAZE.getSkill());
				attackPlayer((L2Attackable) npc, player);
