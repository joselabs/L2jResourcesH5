...
public class FeedableBeasts extends L2AttackableAIScript
{
	...
	{
		"So what do you think it is like to be tamed?",
		"Whenever I see spice, I think I will miss your hand that used to feed it to me.",
		"Don't go to the village. I don't have the strength to follow you.",
	...
	{
		super(-1, FeedableBeasts.class.getSimpleName(), "ai");
	...
	private void spawnNext(L2Npc npc, int growthLevel, L2PcInstance player, int food)
	{
		...
		if (Util.contains(TAMED_BEASTS, nextNpcId))
		{
			...
			{
				case 1:
					npc.broadcastPacket(new CreatureSay(nextNpc.getNpcId(), 1, nextNpc.getName(), player.getName() + ", so what do you think it is like to be tamed?"));
					break;
				case 2:
					npc.broadcastPacket(new CreatureSay(nextNpc.getNpcId(), 1, nextNpc.getName(), "I guess it's just my animal magnetism.?"));
					break;
			}
			// @formatter:off
			/*
	...
	public String onSkillSee(L2Npc npc, L2PcInstance caster, L2Skill skill, L2Object[] targets, boolean isSummon)
	{
		...
		if (_GrowthCapableMobs.containsKey(npcId))
		{
			...
			String message = TEXT[growthLevel][getRandom(TEXT[growthLevel].length)];
			if (getRandom(100) < 20)
			{
				// rare random talk...
				npc.sendPacket(new CreatureSay(npc.getNpcId(), 1, npc.getName(), message));
			}
		...
		else if (Util.contains(TAMED_BEASTS, npcId) && (npc instanceof L2TamedBeastInstance))
		{
			...
			{
				beast.onReceiveFood();
				String message = TAMED_TEXT[getRandom(TAMED_TEXT.length)];
				if (getRandom(100) < 99)
				{
					// rare random talk...
					beast.sendPacket(new CreatureSay(beast.getNpcId(), 1, beast.getName(), message));
				}
			}
		}
		return super.onSkillSee(npc, caster, skill, targets, isSummon);
