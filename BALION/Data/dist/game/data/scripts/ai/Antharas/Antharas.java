...
/**
 * Antharas AI BY L2jPrivateDevelopersTeam
 */
public class Antharas extends L2AttackableAIScript
{
	...
	// Item
	private static final int STONE = 3865; // Portal Stone ? Really remove from inv? Its annoying :-D
	// Skill
	...
	// Zone
	private static final L2BossZone zone = ZoneManager.getInstance().getZoneById(70050, L2BossZone.class); // Antharas Nest zone
	// Status
	...
	// Configurations
	private static int MAXIMUM_PLAYERS = Config.ANTHARAS_MAXIMUM_PLAYERS;
	private static int MAXIMUM_MINIONS = Config.ANTHARAS_MAXIMUM_MINIONS;
	private static int SPAWN_INTERVAL = Config.ANTHARAS_INTERVAL_OF_SPAWN;
	private static int SPAWN_RANDOM = Config.ANTHARAS_RANDOM_SPAWN;
	private static long WAIT_TIME = Config.ANTHARAS_WAIT_TIME;
	// Misc
	...
	public Antharas()
	{
		super(-1, Antharas.class.getSimpleName(), "ai");
		StartNpcs(HEART, CUBE);
		TalkNpcs(HEART, CUBE);
		FirstTalkNpcs(HEART);
		SpawnNpcs(29077, 29078, 29079, 29080, 29081, 29082, 29083, 29084, 29085, 29086, 29087, 29088, 29089, 29090, 29091, 29092, 29093, 29094);
		SpawnNpcs(ANTHARAS);
		MoveFinishedNpcs(BOMBER);
		AggroRangeEnterNpcs(BOMBER);
		SpellFinishedNpcs(ANTHARAS);
		AttackNpcs(ANTHARAS, BOMBER, BEHEMOTH, TERASQUE);
		KillNpcs(ANTHARAS, TERASQUE, BEHEMOTH);
		...
		switch (getStatus())
		{
			case ALIVE:
			{
				_antharas = (L2GrandBossInstance) addSpawn(ANTHARAS, 185524, 114757, -8219, 34815, false, 0, false, 0);
			...
			case WAITING:
			{
				...
				_antharas.setCurrentHpMp(curr_hp, curr_mp);
				GrandBossManager.getInstance().addBoss(_antharas);
				startQuestTimer("SPAWN_ANTHARAS", WAIT_TIME, null, null);
				break;
			}
			case IN_FIGHT:
			{
				_antharas = (L2GrandBossInstance) addSpawn(ANTHARAS, loc_x, loc_y, loc_z, heading, false, 0, false, 0);
				_antharas.setCurrentHpMp(curr_hp, curr_mp);
				GrandBossManager.getInstance().addBoss(_antharas);
			...
			case DEAD:
			{
				...
				else
				{
					setStatus(ALIVE);
					_antharas = (L2GrandBossInstance) addSpawn(ANTHARAS, 185524, 114757, -8219, 34815, false, 0, false, 0);
					addSpawn(ANTHARAS, 185708, 114298, -8221, 0, false, 0);
					GrandBossManager.getInstance().addBoss(_antharas);
				}
				break;
			}
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		switch (event)
		{
			case "enter":
			{
				...
				else if (zone.getPlayersInside().size() >= MAXIMUM_PLAYERS)
				{
				...
				else if (player.isInParty())
				{
					...
					else if (members.size() > (MAXIMUM_PLAYERS - zone.getPlayersInside().size()))
					{
						htmltext = "13001-04.html";
					}
					else
			...
			case "SPAWN_ANTHARAS":
			{
				_antharas.teleToLocation(181647, 114867, -7676, 0);
			...
			case "CAMERA_1":
			{
				zone.broadcastPacket(new SpecialCamera(npc.getObjectId(), 700, 13, -19, 0, 20000, 0, 0, 1, 0));
				startQuestTimer("CAMERA_2", 3000, npc, null);
				break;
			}
			case "CAMERA_2":
			{
				zone.broadcastPacket(new SpecialCamera(npc.getObjectId(), 700, 13, 0, 6000, 20000, 0, 0, 1, 0));
				startQuestTimer("CAMERA_3", 10000, npc, null);
				break;
			}
			case "CAMERA_3":
			{
				zone.broadcastPacket(new SpecialCamera(npc.getObjectId(), 3700, 0, -3, 0, 10000, 0, 0, 1, 0));
			...
			case "CAMERA_4":
			{
				zone.broadcastPacket(new SpecialCamera(npc.getObjectId(), 1100, 0, -3, 22000, 30000, 0, 0, 1, 0));
				startQuestTimer("CAMERA_5", 10800, npc, null);
				break;
			}
			case "CAMERA_5":
			{
				zone.broadcastPacket(new SpecialCamera(npc.getObjectId(), 1100, 0, -3, 300, 7000, 0, 0, 1, 0));
			...
			case "START_MOVE":
			{
				for (L2PcInstance players : npc.getKnownList().getKnownPlayersInRadius(4000))
				{
					if (players.isHero())
					{
						zone.broadcastPacket(new ExShowScreenMessage(player.getName() + " you cannot hope to defeat me with your meager strenght", 4000));
						break;
					}
				}
				npc.getAI().setIntention(CtrlIntention.AI_INTENTION_MOVE_TO, new L2CharPosition(179011, 114871, -7704, 0));
			...
			case "SET_REGEN":
			{
				if (npc != null)
				{
					...
					{
						if (!npc.isAffected(ANTH_REGEN_4.getSkillId()))
						{
					...
					{
						if (!npc.isAffected(ANTH_REGEN_3.getSkillId()))
						{
					...
					{
						if (!npc.isAffected(ANTH_REGEN_2.getSkillId()))
						{
							npc.doCast(ANTH_REGEN_2.getSkill());
						}
					}
					else if (!npc.isAffected(ANTH_REGEN_1.getSkillId()))
					{
					...
					if ((_lastAttack + 900000) < System.currentTimeMillis())
					{
						...
						for (L2Character charInside : zone.getCharactersInside())
						{
							if (charInside != null)
							{
								if (charInside.isNpc())
								{
									if (charInside.getNpcId() == ANTHARAS)
									{
			...
			case "SPAWN_MINION":
			{
				if ((minionMultipler > 1) && (_minionCount < (MAXIMUM_MINIONS - (minionMultipler * 2))))
				{
				...
				else if (_minionCount < (MAXIMUM_MINIONS - 2))
				{
					addSpawn(BEHEMOTH, npc, true);
					addSpawn(TERASQUE, npc, true);
					_minionCount += 2;
				}
				else if (_minionCount < (MAXIMUM_MINIONS - 1))
				{
			...
			case "TID_USED_FEAR":
			{
				if ((npc != null) && (sandStorm == 0))
				{
					...
					npc.getAI().setIntention(CtrlIntention.AI_INTENTION_MOVE_TO, new L2CharPosition(177648, 114816, -7735, 0));
			...
			case "TID_FEAR_MOVE_TIMEOVER":
			{
				if ((sandStorm == 1) && (npc.getX() == 177648) && (npc.getY() == 114816))
				{
					...
					addSpawn(29077, 177229, 113298, -7735, 0, false, 0); // antaras_clear_npc_1
					addSpawn(29078, 176707, 113585, -7735, 0, false, 0); // antaras_clear_npc_2
					addSpawn(29079, 176385, 113889, -7735, 0, false, 0); // antaras_clear_npc_3
					addSpawn(29080, 176082, 114241, -7735, 0, false, 0); // antaras_clear_npc_4
					addSpawn(29081, 176066, 114802, -7735, 0, false, 0); // antaras_clear_npc_5
					addSpawn(29082, 176095, 115313, -7735, 0, false, 0); // antaras_clear_npc_6
					addSpawn(29083, 176425, 115829, -7735, 0, false, 0); // antaras_clear_npc_7
					addSpawn(29084, 176949, 116378, -7735, 0, false, 0); // antaras_clear_npc_8
					addSpawn(29085, 177655, 116402, -7735, 0, false, 0); // antaras_clear_npc_9
					addSpawn(29086, 178248, 116395, -7735, 0, false, 0); // antaras_clear_npc_10
					addSpawn(29087, 178706, 115998, -7735, 0, false, 0); // antaras_clear_npc_11
					addSpawn(29088, 179208, 115452, -7735, 0, false, 0); // antaras_clear_npc_12
					addSpawn(29089, 179191, 115079, -7735, 0, false, 0); // antaras_clear_npc_13
					addSpawn(29090, 179221, 114546, -7735, 0, false, 0); // antaras_clear_npc_14
					addSpawn(29091, 178916, 113925, -7735, 0, false, 0); // antaras_clear_npc_15
					addSpawn(29092, 178782, 113814, -7735, 0, false, 0); // antaras_clear_npc_16
					addSpawn(29093, 178419, 113417, -7735, 0, false, 0); // antaras_clear_npc_17
					addSpawn(29094, 177855, 113282, -7735, 0, false, 0); // antaras_clear_npc_18
				}
				else if (sandStorm == 1)
				{
					if (moveChance <= 3)
			...
			case "DESPAWN_MINIONS":
			{
				if (getStatus() == IN_FIGHT)
				{
					...
					for (L2Character charInside : zone.getCharactersInside())
					{
						if ((charInside != null) && charInside.isNpc() && ((charInside.getNpcId() == BEHEMOTH) || (charInside.getNpcId() == TERASQUE)))
						{
							charInside.deleteMe();
						}
					}
	...
	public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isSummon, L2Skill skill)
	{
		...
		if (npc.getNpcId() == BOMBER)
		{
			if (npc.calculateDistance(attacker.getLocationXYZ(), true, false) < 230)
			{
				npc.doCast(DISPEL_BOM.getSkill());
				npc.doDie(attacker);
			}
		...
		else if (npc.getNpcId() == ANTHARAS)
		{
			...
			{
				_log.info(getClass().getSimpleName() + ": Player " + attacker.getName() + " attacked Antharas in invalid conditions!");
				attacker.teleToLocation(80464, 152294, -3534);
			}
			
			if ((attacker.getMountType() == 1) && !attacker.isAffected(ANTH_ANTI_STRIDER.getSkillId()))
			{
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isSummon)
	{
		if (zone.isCharacterInZone(killer))
		{
			if (npc.getNpcId() == ANTHARAS)
			{
				...
				zone.broadcastPacket(new SpecialCamera(npc.getObjectId(), 1200, 20, -10, 0, 13000, 0, 0, 0, 0));
				zone.broadcastPacket(new PlaySound("BS01_D"));
				addSpawn(CUBE, 177615, 114941, -7709, 0, false, 900000);
				// Calculate Min and Max respawn times randomly.
				long respawnTime = SPAWN_INTERVAL + getRandom(-SPAWN_RANDOM, SPAWN_RANDOM);
				respawnTime *= 3600000;
		...
		else
		{
			try
			{
				for (int i = 1; i <= 6; i++)
				{
					...
					bomber.getAI().setIntention(CtrlIntention.AI_INTENTION_MOVE_TO, new L2CharPosition(x, y, npc.getZ(), 0));
				}
				npc.deleteMe();
			}
			catch (Exception e)
	...
	private void manageSkills(L2Npc npc)
	{
		...
		if ((attacker_1 == null) || (npc.calculateDistance(attacker_1.getLocationXYZ(), true, false) > 9000) || attacker_1.isDead())
		{
			attacker_1_hate = 0;
		}
		
		if ((attacker_2 == null) || (npc.calculateDistance(attacker_2.getLocationXYZ(), true, false) > 9000) || attacker_2.isDead())
		{
			attacker_2_hate = 0;
		}
		
		if ((attacker_3 == null) || (npc.calculateDistance(attacker_3.getLocationXYZ(), true, false) > 9000) || attacker_3.isDead())
		{
			attacker_3_hate = 0;
		}
		...
		if (i2 > 0)
		{
			...
			final double distance_c2 = npc.calculateDistance(c2.getLocationXYZ(), true, false);
			final double direction_c2 = npc.calculateDirectionTo(c2.getLocationXYZ());
