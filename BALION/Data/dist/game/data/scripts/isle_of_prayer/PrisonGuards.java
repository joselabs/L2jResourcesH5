...
public class PrisonGuards extends L2AttackableAIScript
{
	...
	public PrisonGuards()
	{
		super(-1, PrisonGuards.class.getSimpleName(), "isle_of_prayer");
		int[] mob =
		{
			GUARD1,
			GUARD2
	...
	public String onAttack(L2Npc npc, L2PcInstance player, int damage, boolean isPet)
	{
		...
		{
			if ((player.getQuestState(IOPRace.class.getSimpleName()) != null) && (player.getQuestState(IOPRace.class.getSimpleName()).getInt(GUARDVARS[_guards.get(npc)]) != 1))
			{
				player.getQuestState(IOPRace.class.getSimpleName()).set(GUARDVARS[_guards.get(npc)], "1");
				player.getQuestState(IOPRace.class.getSimpleName()).giveItems(STAMP, 1);
			}
		}
	...
	private void castDebuff(L2Npc npc, L2Character player, int effectId, boolean isSummon, boolean fromAttack, boolean isSpell)
	{
		if (fromAttack)
		{
			String npcString = (npc.getNpcId() == GUARD1 ? "It's not easy to obtain." : "You're out of your mind coming here...");
			npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), npcString));
		}
