...
public final class Q00211_TrialOfTheChallenger extends Quest
{
	...
	public Q00211_TrialOfTheChallenger()
	{
		...
		StartNpcs(KASH);
		TalkNpcs(FILAUR, KASH, MARTIAN, RALDO, CHEST_OF_SHYSLASSYS, MARKETEER_OF_MAMMON);
		KillNpcs(SHYSLASSYS, GORR, BARAHAM, QUEEN_OF_SUCCUBUS);
	...
	public String onTalk(L2Npc npc, L2PcInstance talker)
	{
		...
		switch (npc.getNpcId())
		{
			case KASH:
			{
				if (qs.isCreated())
				{// WARRIOR_GROUP
					if (!(talker.getClassId().getId() == 1) || !(talker.getClassId().getId() == 19) || !(talker.getClassId().getId() == 32) || !(talker.getClassId().getId() == 45) || !(talker.getClassId().getId() == 47)
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isSummon)
	{
		...
		switch (npc.getNpcId())
		{
			case SHYSLASSYS:
			{
				if (qs.isCond(1))
				{
					if (SpawnTable.getInstance().getSpawns(npc.getNpcId()).size() < 10)
					{
