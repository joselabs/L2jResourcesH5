...
public final class Q00403_PathOfTheRogue extends Quest
{
	...
	public Q00403_PathOfTheRogue()
	{
		...
		StartNpcs(CAPTAIN_BEZIQUE);
		TalkNpcs(CAPTAIN_BEZIQUE, NETI);
		AttackNpcs(20035, 20042, 20045, 20051, 20054, 20060);
		AttackNpcs(CATS_EYE_BANDIT);
		KillNpcs(MONSTER_DROPS.keySet());
		KillNpcs(CATS_EYE_BANDIT);
			...
			switch (npc.getScriptValue())
			{
				case 0:
				{
					...
					else
					{
						...
						{
							attacker.sendPacket(new CreatureSay(attacker.getObjectId(), 1, attacker.getName(), "You childish fool, do you think you can catch me?"));
						}
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isSummon)
	{
		...
		if ((qs != null) && qs.isStarted() && npc.isScriptValue(1) && Util.checkIfInRange(1500, npc, killer, true))
		{
			if (npc.getNpcId() == CATS_EYE_BANDIT)
			{
				npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "I must do something about this shameful incident..."));
			...
			{
				final ItemChanceHolder reward = MONSTER_DROPS.get(npc.getNpcId());
	...
	private boolean checkWeapon(L2PcInstance player)
	{
		...
		return ((weapon != null) && ((weapon.getItemId() == NETIS_BOW) || (weapon.getItemId() == NETIS_DAGGER)));
	}
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		{
			if (npc.getNpcId() == CAPTAIN_BEZIQUE)
			{
