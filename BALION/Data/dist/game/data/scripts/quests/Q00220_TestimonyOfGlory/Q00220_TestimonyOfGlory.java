...
public final class Q00220_TestimonyOfGlory extends Quest
{
	...
	public Q00220_TestimonyOfGlory()
	{
		...
		StartNpcs(PREFECT_VOKIAN);
		TalkNpcs(PREFECT_VOKIAN, PREFECT_KASMAN, SEER_MANAKIA, FLAME_LORD_KAKAI, SEER_TANAPI, BREKA_CHIEF_VOLTAR, ENKU_CHIEF_KEPRA, TUREK_CHIEF_BURAI, LEUNT_CHIEF_HARAK, VUKU_CHIEF_DRIKO, GANDI_CHIEF_CHIANTA);
		KillNpcs(TYRANT, TYRANT_KINGPIN, MARSH_STAKATO_DRONE, GUARDIAN_BASILISK, MANASHEN_GARGOYLE, TIMAK_ORC, TIMAK_ORC_ARCHER, TIMAK_ORC_SOLDIER, TIMAK_ORC_WARRIOR, TIMAK_ORC_SHAMAN, TIMAK_ORC_OVERLORD, TAMLIN_ORC, TAMLIN_ORC_ARCHER, RAGNA_ORC_OVERLORD, RAGNA_ORC_SEER, PASHIKA_SON_OF_VOLTAR, VULTUS_SON_OF_VOLTAR, ENKU_ORC_OVERLORD, MAKUM_BUGBEAR_THUG, REVENANT_OF_TANTOS_CHIEF);
		AttackNpcs(RAGNA_ORC_OVERLORD, RAGNA_ORC_SEER, REVENANT_OF_TANTOS_CHIEF);
		...
		switch (event)
		{
			...
			case "30615-04.html":
			{
				if (hasQuestItems(player, MANAKIA_1ST_LETTER))
				{
					...
					attackPlayer(addSpawn(npc, PASHIKA_SON_OF_VOLTAR, npc.getLocationXYZ(), true, 200000), player);
					attackPlayer(addSpawn(npc, VULTUS_SON_OF_VOLTAR, npc.getLocationXYZ(), true, 200000), player);
					htmltext = event;
				}
				break;
			}
			case "30616-04.html":
			{
				if (hasQuestItems(player, MANAKIA_2ND_LETTER))
				{
					...
					attackPlayer(addSpawn(npc, ENKU_ORC_OVERLORD, npc.getLocationXYZ(), true, 200000), player);
					attackPlayer(addSpawn(npc, ENKU_ORC_OVERLORD, npc.getLocationXYZ(), true, 200000), player);
					attackPlayer(addSpawn(npc, ENKU_ORC_OVERLORD, npc.getLocationXYZ(), true, 200000), player);
					attackPlayer(addSpawn(npc, ENKU_ORC_OVERLORD, npc.getLocationXYZ(), true, 200000), player);
					htmltext = event;
			...
			case "30617-03.html":
			{
				if (hasQuestItems(player, KASMANS_2ND_LETTER))
				{
					...
					attackPlayer(addSpawn(npc, MAKUM_BUGBEAR_THUG, npc.getLocationXYZ(), true, 200000), player);
					attackPlayer(addSpawn(npc, MAKUM_BUGBEAR_THUG, npc.getLocationXYZ(), true, 200000), player);
					htmltext = event;
				}
				break;
				...
				case RAGNA_ORC_SEER:
				{
					switch (npc.getScriptValue())
					{
						case 0:
						{
							...
							{
								npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Is it a lackey of Kakai?!"));
				...
				case REVENANT_OF_TANTOS_CHIEF:
				{
					switch (npc.getScriptValue())
					{
						case 0:
						{
							...
							if (!hasQuestItems(attacker, SCEPTER_OF_TANTOS))
							{
								npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "How regretful! Unjust dishonor!"));
								npc.setScriptValue(1);
							}
							break;
						}
						case 1:
						{
							...
							{
								npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Indignant and unfair death!"));
								npc.setScriptValue(2);
							}
							break;
						}
			...
			switch (npc.getNpcId())
			{
				...
				case RAGNA_ORC_SEER:
				{
					if (hasQuestItems(killer, TANAPIS_ORDER) && !hasQuestItems(killer, SCEPTER_OF_TANTOS))
					{
						...
						npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Too late!"));
					}
					break;
				}
				...
				case REVENANT_OF_TANTOS_CHIEF:
				{
					if (hasQuestItems(killer, TANAPIS_ORDER) && !hasQuestItems(killer, SCEPTER_OF_TANTOS))
					{
						...
						npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "I'll get revenge someday!!"));
						qs.setCond(10, true);
					}
					break;
				}
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		if (qs.isCreated())
		{
			if (npc.getNpcId() == PREFECT_VOKIAN)
			{
				if (player.getRace() == Race.Orc)
				{
					if (player.getLevel() >= MIN_LEVEL)
					{
						if ((player.getClassId().getId() == 50) || (player.getClassId().getId() == 45) || (player.getClassId().getId() == 47))
						{
