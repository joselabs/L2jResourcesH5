...
public final class Q00223_TestOfTheChampion extends Quest
{
	...
	public Q00223_TestOfTheChampion()
	{
		...
		StartNpcs(VETERAN_ASCALON);
		TalkNpcs(VETERAN_ASCALON, TRADER_GROOT, CAPTAIN_MOUEN, MASON);
		KillNpcs(HARPY, MEDUSA, WINDSUS, ROAD_SCAVENGER, LETO_LIZARDMAN, LETO_LIZARDMAN_ARCHER, LETO_LIZARDMAN_SOLDIER, LETO_LIZARDMAN_WARRIOR, LETO_LIZARDMAN_SHAMAN, LETO_LIZARDMAN_OCERLORD, BLOODY_AXE_ELITE, HARPY_MATRIARCH, ROAD_COLLECTOR);
		AttackNpcs(HARPY, ROAD_SCAVENGER, BLOODY_AXE_ELITE);
						...
						case 0:
						{
							...
							if (hasQuestItems(attacker, WHITE_ROSE_INSIGNIA) && (getQuestItemsCount(attacker, HARPYS_EGG) < 30))
							{
								if (getRandomBoolean())
								{
									if (getRandom(10) < 7)
									{
										final L2Attackable monster1 = (L2Attackable) addSpawn(HARPY_MATRIARCH, npc.getLocationXYZ(), true, 0, false);
										attackPlayer(monster1, attacker);
									}
									else
									{
										final L2Attackable monster1 = (L2Attackable) addSpawn(HARPY_MATRIARCH, npc.getLocationXYZ(), true, 0, false);
										final L2Attackable monster2 = (L2Attackable) addSpawn(HARPY_MATRIARCH, npc.getLocationXYZ(), true, 0, false);
										attackPlayer(monster1, attacker);
						...
						case 0:
						{
							...
							if (hasQuestItems(attacker, MOUENS_1ST_ORDER) && (getQuestItemsCount(attacker, ROAD_RATMAN_HEAD) < 10))
							{
								if (getRandomBoolean())
								{
									if (getRandom(10) < 7)
									{
										final L2Attackable monster1 = (L2Attackable) addSpawn(ROAD_COLLECTOR, npc.getLocationXYZ(), true, 0, false);
										attackPlayer(monster1, attacker);
									}
									else
									{
										final L2Attackable monster1 = (L2Attackable) addSpawn(ROAD_COLLECTOR, npc.getLocationXYZ(), true, 0, false);
										final L2Attackable monster2 = (L2Attackable) addSpawn(ROAD_COLLECTOR, npc.getLocationXYZ(), true, 0, false);
										attackPlayer(monster1, attacker);
										attackPlayer(monster2, attacker);
									}
								}
					...
					switch (npc.getScriptValue())
					{
						case 0:
						{
							...
							if (hasQuestItems(attacker, IRON_ROSE_RING) && (getQuestItemsCount(attacker, BLOODY_AXE_HEAD) < 10))
							{
								if (getRandomBoolean())
								{
									final L2Attackable monster1 = (L2Attackable) addSpawn(BLOODY_AXE_ELITE, npc.getLocationXYZ(), true, 0, false);
									attackPlayer(monster1, attacker);
								}
							}
							npc.setScriptValue(1);
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		if (qs.isCreated())
		{
			if (npc.getNpcId() == VETERAN_ASCALON)
			{
