...
public final class Q00632_NecromancersRequest extends Quest
{
	...
	public Q00632_NecromancersRequest()
	{
		...
		TalkNpcs(MYSTERIOUS_WIZARD);
		KillNpcs(BRAIN_MONSTERS.keySet());
		KillNpcs(HEART_MONSTERS.keySet());
	...
	public String onKill(L2Npc npc, L2PcInstance player, boolean isSummon)
	{
		...
		if (qs != null)
		{
			if (BRAIN_MONSTERS.containsKey(npc.getNpcId()))
			{
				qs.giveItemRandomly(npc, ZOMBIES_BRAIN, 1, 0, BRAIN_MONSTERS.get(npc.getNpcId()), true);
			}
			else
			{
				qs.giveItemRandomly(npc, VAMPIRES_HEART, 1, 0, HEART_MONSTERS.get(npc.getNpcId()), true);
