...
public final class Q00457_LostAndFound extends Quest
{
	...
	public Q00457_LostAndFound()
	{
		...
		StartNpcs(GUMIEL);
		FirstTalkNpcs(GUMIEL);
		TalkNpcs(GUMIEL);
		SpawnNpcs(GUMIEL);
		KillNpcs(SOLINA_CLAN);
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		switch (event)
		{
			case "32759-06.html":
				...
				break;
			case "talk_time":
				npc.broadcastPacket(new CreatureSay(npc.getObjectId(), Say2.ALL, npc.getName(), "Ah, I think I remember this place!"));
				break;
			case "talk_time2":
				npc.broadcastPacket(new CreatureSay(npc.getObjectId(), Say2.ALL, npc.getName(), "What were you doing here?"));
				startQuestTimer("talk_time3", 10 * 1000, npc, player);
				break;
			case "talk_time3":
				npc.broadcastPacket(new CreatureSay(npc.getObjectId(), Say2.ALL, npc.getName(), "I guess you are the silent type then are you looking for treasure like me? "));
				break;
			case "time_limit":
			...
			case "check":
				...
				if (distance > 1000)
				{
					...
					{
						npc.broadcastPacket(new CreatureSay(npc.getObjectId(), Say2.ALL, npc.getName(), "Hey! Dont go fast!"));
						_count = 1;
					}
					else if (_count == 1)
					{
						npc.broadcastPacket(new CreatureSay(npc.getObjectId(), Say2.ALL, npc.getName(), "Its hard follow"));
						_count = 2;
					}
					else if (_count == 2)
					{
				...
				for (L2Spawn escortSpawn : _escortCheckers)
				{
					...
					if ((escort != null) && npc.isInsideRadius(escort, 1000, false, false))
					{
						...
						npc.broadcastPacket(new CreatureSay(npc.getObjectId(), Say2.ALL, npc.getName(), "Ahh fresh air"));
