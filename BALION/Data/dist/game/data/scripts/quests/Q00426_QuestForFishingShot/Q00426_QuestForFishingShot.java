...
public class Q00426_QuestForFishingShot extends Quest
{
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isSummon)
	{
		...
		if (qs != null)
		{
			if (MOBS_SPECIAL.containsKey(npc.getNpcId()))
			{
				if (Rnd.get(1000) <= MOBS_SPECIAL.get(npc.getNpcId()).chance)
				{
					rewardItems(qs.getPlayer(), SWEET_FLUID, MOBS_SPECIAL.get(npc.getNpcId()).reward + 1);
				}
				else
				{
					rewardItems(qs.getPlayer(), SWEET_FLUID, MOBS_SPECIAL.get(npc.getNpcId()).reward);
				}
				playSound(qs.getPlayer(), QuestSound.ITEMSOUND_QUEST_ITEMGET);
			}
			else
			{
				if (Rnd.get(1000) <= MOBS.get(npc.getNpcId()).chance)
				{
					rewardItems(qs.getPlayer(), SWEET_FLUID, MOBS.get(npc.getNpcId()).reward);
					playSound(qs.getPlayer(), QuestSound.ITEMSOUND_QUEST_ITEMGET);
				}
			}
		}
