...
public final class Q00215_TrialOfThePilgrim extends Quest
{
	...
	public Q00215_TrialOfThePilgrim()
	{
		...
		StartNpcs(HERMIT_SANTIAGO);
		TalkNpcs(HERMIT_SANTIAGO, PRIEST_PETRON, PRIEST_PRIMOS, ANDELLIA, GAURI_TWINKLEROCK, SEER_TANAPI, ELDER_CASIAN, ANCESTOR_MARTANKUS, PRIEST_OF_THE_EARTH_GERALD, WANDERER_DORF, URUHA);
		KillNpcs(LAVA_SALAMANDER, NAHIR, BLACK_WILLOW);
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		if (qs.isCreated())
		{
			if (npc.getNpcId() == HERMIT_SANTIAGO)
			{
				if (!(player.getClassId().getId() == 15) || !(player.getClassId().getId() == 29) || !(player.getClassId().getId() == 42) || !(player.getClassId().getId() == 50))
				{
