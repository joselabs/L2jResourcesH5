	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		else if (st.isCond(1))
		{
			...
			else
			{
				if (getRandom(100) < 50)
				{
					npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "...Grunt... Oh..."));
				}
				else
				{
					npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Grunt... What's... wrong with me..."));
				}
				npc.deleteMe();
				htmltext = null;
				
				final L2Attackable monster = (L2Attackable) addSpawn(WARRIOR_MON, npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), true, 600000);
				monster.ai_attack(monster, player);
				player.sendPacket(new ExShowScreenMessage("The Grave Robber has been filled with Dark Energy and is attacking on you!", 5000));
			}
		}
