...
public final class Q00229_TestOfWitchcraft extends Quest
{
	...
	public Q00229_TestOfWitchcraft()
	{
		...
		{
			StartNpcs(SHADOW_ORIM);
			TalkNpcs(SHADOW_ORIM, GROCER_LARA, TRADER_ALEXANDRIA, MAGISTER_IKER, PRIEST_VADIN, TRADER_NESTLE, SIR_KLAUS_VASPER, LEOPOLD, MAGISTER_KAIRA, WARDEN_RODERIK, WARDEN_ENDRIGO, FISHER_EVERT);
			KillNpcs(DIRE_WYRM, ENCHANTED_STONE_GOLEM, LETO_LIZARDMAN, LETO_LIZARDMAN_ARCHER, LETO_LIZARDMAN_SOLDIER, LETO_LIZARDMAN_WARRIOR, LETO_LIZARDMAN_SHAMAN, LETO_LIZARDMAN_OVERLORD, TAMLIN_ORC, TAMLIN_ORC_ARCHER, NAMELESS_REVENANT, SKELETAL_MERCENARY, DREVANUL_PRINCE_ZERUEL);
			AttackNpcs(NAMELESS_REVENANT, SKELETAL_MERCENARY, DREVANUL_PRINCE_ZERUEL);
		...
		switch (event)
		{
			...
			case "30630-14.htm":
			{
				if (hasQuestItems(player, ALEXANDRIAS_BOOK))
				{
					...
					qs.setCond(4, true);
					addSpawn(DREVANUL_PRINCE_ZERUEL, npc.getLocationXYZ(), true, 0, false);
					htmltext = event;
				}
				break;
			}
			...
			switch (npc.getNpcId())
			{
				case NAMELESS_REVENANT:
				{
					if (npc.isScriptValue(0) && hasQuestItems(attacker, ALEXANDRIAS_BOOK, LARAS_MEMO) && !hasQuestItems(attacker, AKLANTOTH_3RD_GEM))
					{
						...
						npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "I absolutely cannot give it to you! It is my precious jewel!"));
					}
					break;
				}
				...
				case DREVANUL_PRINCE_ZERUEL:
				{
					...
					{
						npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "I'll take your lives later!"));
						npc.deleteMe();
						qs.setCond(5, true);
					}
					else if (hasQuestItems(attacker, ORIMS_INSTRUCTIONS, BRIMSTONE_2ND, SWORD_OF_BINDING, SOULTRAP_CRYSTAL))
					{
						...
						{
							npc.setScriptValue(1);
							npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "That sword is really...!"));
						}
					}
					break;
				}
			...
			switch (npc.getNpcId())
			{
				...
				case DREVANUL_PRINCE_ZERUEL:
				{
					if (hasQuestItems(killer, ORIMS_INSTRUCTIONS, BRIMSTONE_2ND, SWORD_OF_BINDING, SOULTRAP_CRYSTAL))
					{
						...
						{
							npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "No! I haven't completely finished the command for destruction and slaughter yet!!!"));
	...
	private boolean checkWeapon(L2PcInstance player)
	{
		...
		return ((weapon != null) && ((weapon.getItemId() == SWORD_OF_BINDING)));
	}
}