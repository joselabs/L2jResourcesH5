...
public final class Q00409_PathOfTheElvenOracle extends Quest
{
	...
	public Q00409_PathOfTheElvenOracle()
	{
		...
		StartNpcs(PRIEST_MANUEL);
		TalkNpcs(PRIEST_MANUEL, ALLANA, PERRIN);
		KillNpcs(TAMIL, lIZARDMAN_WARRIOR, LIZARDMAN_SCOUT, LIZARDMAN_SOLDIER);
		AttackNpcs(TAMIL, lIZARDMAN_WARRIOR, LIZARDMAN_SCOUT, LIZARDMAN_SOLDIER);
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		switch (event)
		{
			...
			{
				qs.setMemoState(2);
				final L2Attackable monster1 = (L2Attackable) addSpawn(lIZARDMAN_WARRIOR, npc.getLocationXYZ(), true, 0, false);
				final L2Attackable monster2 = (L2Attackable) addSpawn(LIZARDMAN_SCOUT, npc.getLocationXYZ(), true, 0, false);
				final L2Attackable monster3 = (L2Attackable) addSpawn(LIZARDMAN_SOLDIER, npc.getLocationXYZ(), true, 0, false);
			...
			case "replay_2":
			{
				...
				{
					qs.setMemoState(3);
					final L2Attackable monster = (L2Attackable) addSpawn(TAMIL, npc.getLocationXYZ(), true, 0, true);
			...
			switch (npc.getScriptValue())
			{
				case 0:
				{
					switch (npc.getNpcId())
					{
						...
						{
							npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "The sacred flame is ours!"));
							break;
						}
						...
						case TAMIL:
						{
							npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "As you wish, master!"));
							break;
						}
					}
		...
		if ((qs != null) && qs.isStarted() && npc.isScriptValue(1) && Util.checkIfInRange(1500, npc, killer, true))
		{
			switch (npc.getNpcId())
			{
				case lIZARDMAN_WARRIOR:
				{
					...
					{
						npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Arrghh...we shall never.. surrender..."));
