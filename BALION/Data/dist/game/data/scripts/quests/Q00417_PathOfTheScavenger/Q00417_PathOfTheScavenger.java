...
public final class Q00417_PathOfTheScavenger extends Quest
{
	...
	public Q00417_PathOfTheScavenger()
	{
		...
		StartNpcs(COLLECTOR_PIPI);
		TalkNpcs(COLLECTOR_PIPI, WAREHOUSE_KEEPER_RAUT, TRADER_MION, TRADER_SHARI, HEAD_BLACKSMITH_BRONK, PRIEST_OF_THE_EARTH_ZIMENF, MASTER_TOMA, TORAI, WAREHOUSE_CHIEF_YASENI);
		AttackNpcs(HUNTER_TARANTULA, PLUNDER_TARANTULA, HUNTER_BEAR, HONEY_BEAR);
		KillNpcs(HUNTER_TARANTULA, PLUNDER_TARANTULA, HUNTER_BEAR, HONEY_BEAR);
			...
			switch (npc.getNpcId())
			{
				case HUNTER_BEAR:
				{
					if (npc.isScriptValue(1) && firstAttacker && hasQuestItems(killer, BEAR_PICTURE) && (getQuestItemsCount(killer, HONEY_JAR) < 5))
					{
						...
						{
							addSpawn(HONEY_BEAR, npc.getLocationXYZ(), true, 0, true);
							qs.set(FLAG, 0);
						}
						else
						{
				...
				case HONEY_BEAR:
				{
					if (npc.isScriptValue(2) && firstAttacker && (((L2Attackable) npc).getIsSpoiledBy() == 1) && hasQuestItems(killer, BEAR_PICTURE))
					{
				...
				case PLUNDER_TARANTULA:
				{
					if (npc.isScriptValue(2) && firstAttacker && (((L2Attackable) npc).getIsSpoiledBy() == 1) && hasQuestItems(killer, TARANTULA_PICTURE))
					{
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		{
			if (npc.getNpcId() == COLLECTOR_PIPI)
			{
