...
public final class Q00219_TestimonyOfFate extends Quest
{
	...
	public Q00219_TestimonyOfFate()
	{
		...
		StartNpcs(MAGISTER_KAIRA);
		TalkNpcs(MAGISTER_KAIRA, MAGISTER_ROA, WAREHOUSE_KEEPER_NORMAN, TETRARCH_THIFIELL, ARKENIA, MASTER_IXIA, ALDERS_SPIRIT, BROTHER_METHEUS, BLOODY_PIXY, BLIGHT_TREANT);
		KillNpcs(HANGMAN_TREE, MARSH_STAKATO, MEDUSA, TYRANT, TYRANT_KINGPIN, DEAD_SEEKER, MARSH_STAKATO_WORKER, MARSH_STAKATO_SOLDIER, MARSH_SPIDER, MARSH_STAKATO_DRONE, BREKA_ORC_OVERLORD, GRANDIS, LETO_LIZARDMAN_OVERLORD, KARUL_BUGBEAR, BLACK_WILLOW_LURKER);
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		if (qs.isCreated())
		{
			if (npc.getNpcId() == MAGISTER_KAIRA)
			{
				if (player.getRace() == Race.DarkElf)
				{
					if (player.getLevel() >= MIN_LEVEL)
					{
						if ((player.getClassId().getId() == 35) || (player.getClassId().getId() == 32) || (player.getClassId().getId() == 39) || (player.getClassId().getId() == 42))
						{
