...
public final class Q00065_CertifiedSoulBreaker extends Quest
{
	...
	public Q00065_CertifiedSoulBreaker()
	{
		...
		StartNpcs(GRAND_MASTER_VITUS);
		TalkNpcs(GRAND_MASTER_VITUS, CAPTAIN_LUCAS, JACOB, GUARD_HARLAN, GUARD_XABER, GUARD_LIAM, GUARD_VESA, GUARD_ZEROME, WHARF_MANAGER_FELTON, KEKROPUS, VICE_HIERARCH_CASCA, GRAND_MASTER_HOLST, GRAND_MASTER_MELDINA, KATENAR, CARGO_BOX, SUSPICIOUS_MAN);
		KillNpcs(WYRM, GUARDIAN_ANGEL);
		SpawnNpcs(GUARDIAN_ANGEL, SUSPICIOUS_MAN);
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		else if ("DESPAWN_70".equals(event))
		{
			...
			if (npc0 != null)
			{
				if (npc0.getVariables().getBoolean("SPAWNED"))
				{
					...
					if (c0 != null)
					{
						npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), c0.getName() + "! I will be back soon. Stay there and don't you dare wander off!"));
					}
				}
			}
			npc.deleteMe();
		...
		switch (event)
		{
			...
			case "32213-04.htm":
			{
				...
				{
					if ((player.getClassId().getId() == 125) || (player.getClassId().getId() == 126))
					{
						htmltext = event;
					}
				}
			...
			case "32199-02.html":
			{
				if (qs.isMemoState(4))
				{
					qs.setMemoState(5);
					qs.setCond(5, true);
					addSpawn(SUSPICIOUS_MAN, SUSPICIOUS_SPAWN, false, 0);
					htmltext = event;
				}
				break;
			}
			...
			switch (npc.getNpcId())
			{
				...
				case GUARDIAN_ANGEL:
				{
					...
					if (killer == c0)
					{
						if (c0 != null)
						{
							if (qs.isMemoState(12))
							{
								...
								qs.setMemoState(13);
								npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Grr. I've been hit..."));
							}
						}
					}
					else
					{
						...
						npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Grr! Who are you and why have you stopped me?"));
					}
				}
			}
		}
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		if (qs.isCreated())
		{
			if (npc.getNpcId() == GRAND_MASTER_VITUS)
			{
	...
	{
		if (npc.getNpcId() == SUSPICIOUS_MAN)
		{
			startQuestTimer("DESPAWN_5", 5000, npc, null);
			npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Drats! How could I be so wrong??"));
			npc.setIsRunning(true);
			npc.getAI().setIntention(CtrlIntention.AI_INTENTION_MOVE_TO, MOVE_TO);
		}
		else if (npc.getNpcId() == GUARDIAN_ANGEL)
		{
			final L2PcInstance c0 = npc.getVariables().getObject("player0", L2PcInstance.class);
			startQuestTimer("DESPAWN_70", 70000, npc, null);
			if (c0 != null)
			{
				npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), c0.getName() + " Step back from the confounded box! I will take it myself!"));
			}
		}
