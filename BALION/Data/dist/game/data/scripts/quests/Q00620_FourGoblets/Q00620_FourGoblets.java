...
public class Q00620_FourGoblets extends Quest
{
	...
	public Q00620_FourGoblets()
	{
		...
		StartNpcs(NAMELESS_SPIRIT);
		TalkNpcs(NAMELESS_SPIRIT, GHOST_OF_WIGOTH_1, GHOST_OF_WIGOTH_2, GHOST_CHAMBERLAIN_OF_ELMOREDEN_1, CONQUERORS_SEPULCHER_MANAGER, EMPERORS_SEPULCHER_MANAGER, GREAT_SAGES_SEPULCHER_MANAGER, JUDGES_SEPULCHER_MANAGER);
		KillNpcs(HALISHA_ALECTIA, HALISHA_TISHAS, HALISHA_MEKARA, HALISHA_MORIGUL);
		KillNpcs(MOB1.keySet());
		KillNpcs(MOB2.keySet());
		KillNpcs(MOB3.keySet());
		...
		switch (npc.getNpcId())
		{
			...
			default:
			{
				...
				if (st != null)
				{
					...
					{
						final int itemCount = ((getRandom(100) < MOB2.get(npc.getNpcId())) ? 2 : 1);
						st.giveItemRandomly(npc, SEALED_BOX, itemCount, 0, 1.0, true);
					}
					else
					{
						final int itemCount = ((getRandom(100) < MOB3.get(npc.getNpcId())) ? 5 : 4);
						st.giveItemRandomly(npc, SEALED_BOX, itemCount, 0, 1.0, true);
					}
				}
				break;
