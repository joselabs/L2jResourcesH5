...
public final class Q00416_PathOfTheOrcShaman extends Quest
{
	...
	public Q00416_PathOfTheOrcShaman()
	{
		...
		StartNpcs(TATARU_ZU_HESTUI);
		TalkNpcs(TATARU_ZU_HESTUI, UMOS, MOIRA, DEAD_LEOPARDS_CARCASS, DUDA_MARA_TOTEM_SPIRIT, HESTUI_TOTEM_SPIRIT, TOTEM_SPIRIT_OF_GANDI);
		KillNpcs(MOBS.keySet());
		KillNpcs(BLACK_LEOPARD);
	...
	public String onKill(L2Npc npc, L2PcInstance player, boolean isSummon)
	{
		...
		if (npc.getNpcId() == BLACK_LEOPARD)
		{
			switch (st.getMemoState())
			{
				...
				case 103:
				{
					...
					if (getRandom(100) < 66)
					{
						npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "My dear friend of " + st.getPlayer().getName() + ", who has gone on ahead of me!"));
					}
					break;
				}
				case 105:
				{
					...
					if (getRandom(100) < 66)
					{
						npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Listen to Tejakar Gandi, young Oroka! The spirit of the slain leopard is calling you, " + st.getPlayer().getName() + "!"));
					}
					break;
				}
				case 107:
		...
		final ItemChanceHolder item = MOBS.get(npc.getNpcId());
		if (item.getCount() == st.getCond())
		{
			...
			{
				if ((npc.getNpcId() == 20038) || (npc.getNpcId() == 20043))
				{
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		if (st.isCreated())
		{
			if (npc.getNpcId() == TATARU_ZU_HESTUI)
			{
