...
public class Q00025_HidingBehindTheTruth extends Quest
{
	...
	public Q00025_HidingBehindTheTruth()
	{
		...
		StartNpcs(PRIEST_BENEDICT);
		TalkNpcs(HIGH_PRIEST_AGRIPEL, PRIEST_BENEDICT, MYSTERIOUS_WIZARD, TOMBSTONE, MAID_OF_LIDIA, BROKEN_BOOKSHELF2, BROKEN_BOOKSHELF3, BROKEN_BOOKSHELF4, COFFIN);
		registerQuestItems(GEMSTONE_KEY, CONTRACT, TOTEM_DOLL3, TOTEM_DOLL2, LIDAS_DRESS);
		AttackNpcs(TRIOL_PAWN);
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		switch (event)
		{
				...
				if (qs.isMemoState(8))
				{
					if (!hasQuestItems(player, TOTEM_DOLL3))
					{
						...
						if (brokenDeskOwner == 0)
						{
							...
							final L2Npc triyol = addSpawn(TRIOL_PAWN, TRIOL_PAWN_LOC.get(npc.getNpcId()), true, 0);
			...
			case "SAY_TRIYOL":
			{
				npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "That box was sealed by my master, " + player.getName() + "! Don't touch it!"));
				break;
			}
			case "DESPAWN_TRIYOL":
			{
	...
	public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isSummon)
	{
		if (npc.getCurrentHp() <= (0.30 * npc.getMaxHp()))
		{
			...
			if (qs.isMemoState(8) && !hasQuestItems(attacker, TOTEM_DOLL3) && (attacker.getObjectId() == npc.getScriptValue()))
			{
				...
				qs.setCond(8, true);
				npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "You've ended my immortal life! You're protected by the feudal lord, aren't you?"));
				final L2Npc brokenDesk = npc.getVariables().getObject("Q00025", L2Npc.class);
				if (brokenDesk != null)
				{
					brokenDesk.getVariables().set("Q00025", 0);
