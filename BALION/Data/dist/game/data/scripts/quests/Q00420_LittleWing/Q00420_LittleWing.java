...
public final class Q00420_LittleWing extends Quest
{
	...
	public Q00420_LittleWing()
	{
		...
		StartNpcs(COOPER);
		TalkNpcs(MARIA, CRONOS, BYRON, MIMYU, EXARION, ZWOV, KALIBRAN, SUZET, SHAMHAI, COOPER);
		AttackNpcs(DELUXE_STONE_BREAKERS);
		KillNpcs(TOAD_LORD, DEAD_SEEKER, MARSH_SPIDER, BREKA_OVERLORD, ROAD_SCAVENGER, LETO_WARRIOR);
	...
	public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isSummon)
	{
		...
		if ((qs != null) && (getQuestItemsCount(attacker, DELUXE_FAIRY_STONE) > 0) && (getRandom(100) < 30))
		{
			...
			npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "The stone... the Elven stone... broke..."));
		}
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isSummon)
	{
		...
		if (qs != null)
		{
			if (qs.isCond(2) && (npc.getNpcId() == TOAD_LORD))
			{
			...
			else if (qs.isCond(6) && (npc.getNpcId() == qs.getInt("drake_hunt")))
			{
				giveItemRandomly(qs.getPlayer(), npc, EGG_DROPS.get(npc.getNpcId()), 1, 20, 0.5, true);
			}
		}
