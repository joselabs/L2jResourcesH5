...
public final class Q00212_TrialOfDuty extends Quest
{
	...
	public Q00212_TrialOfDuty()
	{
		...
		StartNpcs(HANNAVALT);
		TalkNpcs(HANNAVALT, DUSTIN, SIR_COLLIN_WINDAWOOD, SIR_ARON_TANFORD, SIR_KIEL_NIGHTHAWK, ISAEL_SILVERSHADOW, SPIRIT_OF_SIR_TALIANUS);
		KillNpcs(HANGMAN_TREE, SKELETON_MARAUDER, SKELETON_RAIDER, STRAIN, GHOUL, BREKA_ORC_OVERLORD, LETO_LIZARDMAN, LETO_LIZARDMAN_ARCHER, LETO_LIZARDMAN_SOLDIER, LETO_LIZARDMAN_WARRIOR, LETO_LIZARDMAN_SHAMAN, LETO_LIZARDMAN_OVERLORD, SPIRIT_OF_SIR_HEROD);
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		switch (event)
		{
			case "quest_accept":
			{
				if ((qs.isCreated() && (player.getLevel() >= MIN_LEVEL) && (player.getClassId().getId() == 4)) || (player.getClassId().getId() == 19) || (player.getClassId().getId() == 32))
				{
					qs.startQuest();
					qs.setMemoState(1);
					qs.set("flag", 0);
		...
		switch (npc.getNpcId())
		{
			...
			case SPIRIT_OF_SIR_HEROD:
			{
				if (qs.isMemoState(2))
				{
					...
					if ((weapon != null) && (weapon.getItemId() == OLD_KNIGHTS_SWORD))
					{
	...
	public String onTalk(L2Npc npc, L2PcInstance talker)
	{
		...
		switch (npc.getNpcId())
		{
			case HANNAVALT:
			{
				if (qs.isCreated())
				{
					if (!(talker.getClassId().getId() == 4) || !(talker.getClassId().getId() == 19) || !(talker.getClassId().getId() == 32))
					{
						html = "30109-02.html";
					}
					...
					switch (qs.getMemoState())
					{
						...
						case 14:
						{
							if (hasQuestItems(talker, LETTER_OF_DUSTIN))
							{
								...
								talker.sendPacket(new SocialAction(talker, 3));
