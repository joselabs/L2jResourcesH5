...
public final class Q00225_TestOfTheSearcher extends Quest
{
	...
	public Q00225_TestOfTheSearcher()
	{
		...
		StartNpcs(MASTER_LUTHER);
		TalkNpcs(MASTER_LUTHER, CAPTAIN_ALEX, TYRA, TREE, STRONG_WOODEN_CHEST, MILITIAMAN_LEIRYNN, DRUNKARD_BORYS, BODYGUARD_JAX);
		KillNpcs(HANGMAN_TREE, ROAD_SCAVENGER, GIANT_FUNGUS, DELU_lIZARDMAN_SHAMAN, NEER_BODYGUARD, DELU_CHIEF_KALKIS);
		AttackNpcs(DELU_lIZARDMAN_SHAMAN);
		...
		switch (event)
		{
			...
			case "30627-01a.html":
			{
				if (npc.getSummonedNpcCount() < 5)
				{
					...
					addSpawn(npc, STRONG_WOODEN_CHEST, npc.getLocationXYZ(), true, 0);
					qs.setCond(17, true);
					htmltext = event;
				}
				break;
