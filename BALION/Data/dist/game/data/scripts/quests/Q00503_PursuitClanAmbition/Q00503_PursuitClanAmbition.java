...
public class Q00503_PursuitClanAmbition extends Quest
{
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		else if (event.equalsIgnoreCase("30645-03.htm"))
		{
			...
			List<L2PcInstance> members = player.getClan().getOnlineMember(player.getObjectId());
		...
		else if (event.equalsIgnoreCase("30766-04.htm"))
		{
			...
			spawnedNpc.broadcastPacket(new CreatureSay(spawnedNpc.getObjectId(), 1, spawnedNpc.getName(), "Blood and honor!"));
			spawnedNpc = st.addSpawn(30759, 160665, 21209, -3710, 0, false, 90000);
			spawnedNpc.broadcastPacket(new CreatureSay(spawnedNpc.getObjectId(), 1, spawnedNpc.getName(), "Ambition and power!"));
			spawnedNpc = st.addSpawn(30758, 160665, 21291, -3710, 0, false, 90000);
			spawnedNpc.broadcastPacket(new CreatureSay(spawnedNpc.getObjectId(), 1, spawnedNpc.getName(), "War and death!"));
		}
			...
			if ((cond == droplist.cond) && (getRandom(100) < droplist.chance))
			{
				...
				else
				{
					if (npcId == 27181)
					{
						...
						npc.broadcastPacket(new CreatureSay(spawnedNpc.getObjectId(), 1, spawnedNpc.getName(), "Curse of the gods on the one that defiles the property of the empire!"));
	...
	private void exit(boolean complete, QuestState st)
	{
		...
		try
		{
			List<L2PcInstance> members = st.getPlayer().getClan().getOnlineMember(0);
			for (L2PcInstance i : members)
			{
				if (i == null)
				{
	...
	private int getLeaderVar(QuestState st, String var)
	{
		...
		{
			_log.info("Pursuit of Clan Ambition: cannot read quest states offline clan leader");
		}
		return val;
	}
