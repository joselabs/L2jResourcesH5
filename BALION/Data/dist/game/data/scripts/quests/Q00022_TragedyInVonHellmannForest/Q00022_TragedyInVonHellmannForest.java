...
public final class Q00022_TragedyInVonHellmannForest extends Quest
{
	...
	public Q00022_TragedyInVonHellmannForest()
	{
		...
		KillNpcs(MOBS);
		KillNpcs(SOUL_OF_WELL);
		AttackNpcs(SOUL_OF_WELL);
		StartNpcs(TIFAREN);
		TalkNpcs(INNOCENTIN, TIFAREN, WELL, GHOST_OF_PRIEST, GHOST_OF_ADVENTURER);
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		switch (event)
		{
			...
			case "31334-13.html":
			{
				...
				if (((5 <= cond) && (cond <= 7)) && hasQuestItems(player, CROSS_OF_EINHASAD))
				{
					if (_tifarenOwner == 0)
					{
						...
						ghost2.broadcastPacket(new CreatureSay(ghost2.getObjectId(), 1, ghost2.getName(), "Did you call me " + player.getName() + " ?"));
			...
			case "DESPAWN_GHOST2":
			{
				...
				{
					npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "I'm confused! Maybe it's time to go back."));
				}
				npc.deleteMe();
				break;
			}
