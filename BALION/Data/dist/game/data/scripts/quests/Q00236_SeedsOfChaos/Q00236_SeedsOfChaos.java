...
public final class Q00236_SeedsOfChaos extends Quest
{
	...
	public Q00236_SeedsOfChaos()
	{
		...
		StartNpcs(HIERARCH_KEKROPUS);
		TalkNpcs(HIERARCH_KEKROPUS, KURSTIN, MYSTERIOU_WIZARD, VICE_HIERARCH_MAO, KATENAR, HARKILGAMED, RODENPICULA, ROCK, MOTHER_NORNIL, KATENAR_A, KATENAR_B, HARKILGAMED_A);
		KillNpcs(NEEDLE_STAKATO_DRONE, SHOUT_OF_SPLENDOR, ALLIANCE_OF_SPLENDOR, ALLIANCE_OF_SPLENDOR_1, SIGNET_OF_SPLENDOR, CROWN_OF_SPLENDOR, FANG_OF_SPLENDOR, FANG_OF_SPLENDOR_1, WAILINGOF_SPLENDOR, WAILINGOF_SPLENDOR_1, VAMPIRE_WIZARD, VAMPIRE_WIZARD_A);
		SpawnNpcs(KATENAR, HARKILGAMED, KATENAR_A, KATENAR_B, HARKILGAMED_A);
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		if ("KATENAR_120".equals(event))
		{
			...
			if (npc0 != null)
			{
				if (npc0.getVariables().getBoolean("SPAWNED"))
				{
					...
					if (c0 != null)
					{
						npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Hmm, where did my friend go?"));
					}
				}
			}
			npc.deleteMe();
		...
		else if ("HARKILGAMED_120".equals(event))
		{
			...
			if (npc0 != null)
			{
				...
				{
					npc0.getVariables().set("SPAWNED", false);
					npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Graaah, we're being attacked!"));
				}
			}
			npc.deleteMe();
			return super.onAdvEvent(event, npc, player);
			...
			case "KEITNAR_DESPAWN":
			{
				if (qs.isMemoState(20) && (qs.getMemoStateEx(1) == 0))
				{
					...
					if (player == c0)
					{
						...
						npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Best of luck with your future endeavours."));
						npc.deleteMe();
					}
				}
				break;
			...
			case "HARKILGAMED_DESPAWN":
			{
				if (qs.isMemoState(21))
				{
					...
					if (player == c0)
					{
						...
						npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "In that case, I wish you good luck."));
						npc.deleteMe();
					}
				}
				break;
			...
			case "HARKILGAMED_A_DESPAWN":
			{
				if (qs.isMemoState(30))
				{
					...
					if (player == c0)
					{
						...
						npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Safe travels!"));
						npc.deleteMe();
					}
				}
				break;
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		if (qs.isCreated())
		{
			if (npc.getNpcId() == HIERARCH_KEKROPUS)
			{
	...
	public String onSpawn(L2Npc npc)
	{
		switch (npc.getNpcId())
		{
			case KATENAR:
			{
				...
				if (c0 != null)
				{
					npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), c0.getName() + "! Finally, we meet!"));
				}
				break;
			}
			case HARKILGAMED:
			{
				...
				npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Hmm? Is someone approaching?"));
				break;
			}
			case KATENAR_A:
			{
				...
				if (c0 != null)
				{
					npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), c0.getName() + "! Did you wait for long?"));
				}
				break;
			}
			case KATENAR_B:
			{
				...
				if (c0 != null)
				{
					npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), ", Did you bring what I asked, " + c0.getName()));
				}
				break;
			}
			case HARKILGAMED_A:
			{
				...
				if (c0 != null)
				{
					npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), c0.getName() + ", has everything been found?"));
				}
				break;
			}
		}
