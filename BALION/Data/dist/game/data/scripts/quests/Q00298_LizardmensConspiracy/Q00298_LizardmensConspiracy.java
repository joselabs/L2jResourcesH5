...
public final class Q00298_LizardmensConspiracy extends Quest
{
	...
	public Q00298_LizardmensConspiracy()
	{
		...
		StartNpcs(GUARD_PRAGA);
		TalkNpcs(GUARD_PRAGA, MAGISTER_ROHMER);
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isSummon)
	{
		...
		if (qs != null)
		{
			final ItemChanceHolder item = MONSTERS.get(npc.getNpcId());
	...
	public String onTalk(L2Npc npc, L2PcInstance talker)
	{
		...
		if (qs.isCreated() && (npc.getNpcId() == GUARD_PRAGA))
		{
			html = (talker.getLevel() >= MIN_LVL) ? "30333-01.htm" : "30333-02.htm";
		}
		else if (qs.isStarted())
		{
			if ((npc.getNpcId() == GUARD_PRAGA) && hasQuestItems(talker, PATROLS_REPORT))
			{
				html = "30333-04.html";
			}
			else if (npc.getNpcId() == MAGISTER_ROHMER)
			{
				switch (qs.getCond())
				{
					case 1:
