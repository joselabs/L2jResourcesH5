...
public final class Q00234_FatesWhisper extends Quest
{
	...
	public Q00234_FatesWhisper()
	{
		...
		StartNpcs(MAESTRO_LEORIN);
		TalkNpcs(ZENKIN, CLIFF, MASTER_KASPAR, HEAD_BLACKSMITH_FERRIS, MAESTRO_LEORIN);
		TalkNpcs(COFFER_OF_THE_DEAD, CHEST_OF_KERNON, CHEST_OF_HALLATE, CHEST_OF_GOLKONDA);
		
		KillNpcs(PLATINUM_TRIBE_GRUNT, PLATINUM_TRIBE_ARCHER, PLATINUM_TRIBE_WARRIOR, PLATINUM_TRIBE_SHAMAN, PLATINUM_TRIBE_LORD, GUARDIAN_ANGEL, SEAL_ANGEL, SEAL_ANGEL_R);
		KillNpcs(DOMB_DEATH_CABRIO, KERNON, GOLKONDA_LONGHORN, HALLATE_THE_DEATH_LORD);
		
		SpawnNpcs(COFFER_OF_THE_DEAD, CHEST_OF_KERNON, CHEST_OF_HALLATE, CHEST_OF_GOLKONDA);
		AttackNpcs(BAIUM);
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isSummon)
	{
		switch (npc.getNpcId())
		{
			...
			{
				addSpawn(COFFER_OF_THE_DEAD, npc.getLocationXYZ());
				return super.onKill(npc, killer, isSummon);
			}
			case KERNON:
			{
				addSpawn(CHEST_OF_KERNON, npc.getLocationXYZ());
				return super.onKill(npc, killer, isSummon);
			}
			case GOLKONDA_LONGHORN:
			{
				addSpawn(CHEST_OF_GOLKONDA, npc.getLocationXYZ());
				return super.onKill(npc, killer, isSummon);
			}
			case HALLATE_THE_DEATH_LORD:
			{
				addSpawn(CHEST_OF_HALLATE, npc.getLocationXYZ());
				return super.onKill(npc, killer, isSummon);
			}
		}
	...
	public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isSummon)
	{
		...
		if ((qs != null) && (npc.getNpcId() == BAIUM))
		{
			if ((attacker.getActiveWeaponItem() != null) && (attacker.getActiveWeaponItem().getItemId() == Q_PIPETTE_KNIFE))
			{
				qs.takeItems(Q_PIPETTE_KNIFE, 1);
				qs.giveItems(Q_RED_PIPETTE_KNIFE, 1);
				qs.playSound(QuestSound.ITEMSOUND_QUEST_ITEMGET);
				npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Who dares to try and steal my noble blood?"));
			}
		}
