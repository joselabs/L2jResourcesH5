...
public final class Q00226_TestOfTheHealer extends Quest
{
	...
	public Q00226_TestOfTheHealer()
	{
		...
		StartNpcs(PRIEST_BANDELLOS);
		TalkNpcs(PRIEST_BANDELLOS, MASTER_SORIUS, ALLANA, PERRIN, FATHER_GUPU, ORPHAN_GIRL, WINDY_SHAORING, MYSTERIOUS_DARK_ELF, PIPER_LONGBOW, SLEIN_SHINING_BLADE, CAIN_FLYING_KNIFE, SAINT_KRISTINA, DAURIN_HAMMERCRUSH);
		KillNpcs(LERO_LIZARDMAN_AGENT, LERO_LIZARDMAN_LEADER, LERO_LIZARDMAN_ASSASSIN, LERO_LIZARDMAN_SNIPER, LERO_LIZARDMAN_WIZARD, LERO_LIZARDMAN_LORD, TATOMA);
		...
		switch (event)
		{
			...
			case "30428-02.html":
			{
				if (qs.isMemoState(1) && hasQuestItems(player, REPORT_OF_PERRIN))
				{
					...
					{
						attackPlayer(addSpawn(npc, TATOMA, npc.getLocationXYZ(), true, 200000), player);
					}
				}
				htmltext = event;
				break;
			...
			case "30674-02.html":
			{
				if (qs.isMemoState(6))
				{
					...
					addSpawn(npc, LERO_LIZARDMAN_AGENT, npc.getLocationXYZ(), true, 200000);
					addSpawn(npc, LERO_LIZARDMAN_AGENT, npc.getLocationXYZ(), true, 200000);
					addSpawn(npc, LERO_LIZARDMAN_LEADER, npc.getLocationXYZ(), true, 200000);
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		if (qs.isCreated())
		{
			if (npc.getNpcId() == PRIEST_BANDELLOS)
			{
				if ((player.getClassId().getId() == 15) || (player.getClassId().getId() == 29) || (player.getClassId().getId() == 5) || (player.getClassId().getId() == 19) || (player.getClassId().getId() == 4))
				{
				...
				case MYSTERIOUS_DARK_ELF:
				{
					if (memoState == 8)
					{
						if (hasQuestItems(player, SECRET_LETTER1) && !hasQuestItems(player, SECRET_LETTER2))
						{
							...
							{
								addSpawn(npc, LERO_LIZARDMAN_ASSASSIN, npc.getLocationXYZ(), true, 200000);
								addSpawn(npc, LERO_LIZARDMAN_ASSASSIN, npc.getLocationXYZ(), true, 200000);
								addSpawn(npc, LERO_LIZARDMAN_ASSASSIN, npc.getLocationXYZ(), true, 200000);
								playSound(player, QuestSound.ITEMSOUND_QUEST_BEFORE_BATTLE);
							}
						...
						else if (hasQuestItems(player, SECRET_LETTER1, SECRET_LETTER2) && !hasQuestItems(player, SECRET_LETTER3))
						{
							if (npc.getSummonedNpcCount() < 36)
							{
								addSpawn(npc, LERO_LIZARDMAN_SNIPER, npc.getLocationXYZ(), true, 200000);
								addSpawn(npc, LERO_LIZARDMAN_SNIPER, npc.getLocationXYZ(), true, 200000);
								addSpawn(npc, LERO_LIZARDMAN_SNIPER, npc.getLocationXYZ(), true, 200000);
								playSound(player, QuestSound.ITEMSOUND_QUEST_BEFORE_BATTLE);
							}
						...
						else if (hasQuestItems(player, SECRET_LETTER1, SECRET_LETTER2, SECRET_LETTER3) && !hasQuestItems(player, SECRET_LETTER4))
						{
							if (npc.getSummonedNpcCount() < 36)
							{
								addSpawn(npc, LERO_LIZARDMAN_WIZARD, npc.getLocationXYZ(), true, 200000);
								addSpawn(npc, LERO_LIZARDMAN_WIZARD, npc.getLocationXYZ(), true, 200000);
								addSpawn(npc, LERO_LIZARDMAN_LORD, npc.getLocationXYZ(), true, 200000);
								playSound(player, QuestSound.ITEMSOUND_QUEST_BEFORE_BATTLE);
							}
							qs.setCond(18);
							npc.deleteMe();
