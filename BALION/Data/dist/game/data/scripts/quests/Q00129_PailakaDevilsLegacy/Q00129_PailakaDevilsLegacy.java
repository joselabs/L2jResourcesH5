...
{
	protected static final Logger _log = Logger.getLogger(Q00129_PailakaDevilsLegacy.class.getName());
	//@formatter:off
	// OTHERs
	...
	{
		SURVIVOR,SUPPORTER,ADVENTURER1,ADVENTURER2
	};
	...
	{
		KAMS,HIKORO,ALKASO,GERBERA,LEMATAN,FOLLOWERS,
		TREASURE_BOX,POWDER_KEG,
	...
	public Q00129_PailakaDevilsLegacy()
	{
		...
		FirstTalkNpcs(NPCS);
		TalkNpcs(NPCS);
		AttackNpcs(TREASURE_BOX, POWDER_KEG, LEMATAN);
		KillNpcs(MONSTERS);
		EnterZoneId(ZONE);
		SpawnNpcs(FOLLOWERS);
	...
	public final String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		final QuestState st = player.getQuestState(Q00129_PailakaDevilsLegacy.class.getSimpleName());
		if (st == null)
		{
		...
		switch (event)
		{
			...
			case "lematan_teleport":
				...
				if ((npc.getNpcId() == LEMATAN) && !npc.isMovementDisabled() && !_isOnShip)
				{
					...
					((L2Attackable) npc).setTarget(null);
					// Broadcast Escape
					...
					player.sendPacket(new ExShowScreenMessage("Lematan escaped on ship, follow and finish him!", 5000));
				}
				else
				{
		...
		if ((npc.getNpcId() == LEMATAN) && event.equalsIgnoreCase("lematan_finish_teleport") && !_isOnShip)
		{
			// Teleport Lematan
			...
			npc.getSpawn().setXLocation(84973);
			npc.getSpawn().setYLocation(-208721);
			npc.getSpawn().setZLocation(-3340);
	...
	{
		QuestState st = player.getQuestState(Q00129_PailakaDevilsLegacy.class.getSimpleName());
	...
	private final synchronized void enterInstance(L2PcInstance player)
	{
		...
		{
			final int instanceId = InstanceManager.getInstance().createDynamicInstance("Q00129_PailakaDevilsLegacy.xml");
