...
public final class Q00171_ActsOfEvil extends Quest
{
	...
	public Q00171_ActsOfEvil()
	{
		...
		StartNpcs(GUARD_ALVAH);
		TalkNpcs(GUARD_ALVAH, TRADER_ARODIN, TYRA, NETI, TRADER_ROLENTO, TUREK_CHIEF_BURAI);
		KillNpcs(TUMRAN_BUGBEAR, TUMRAN_BUGBEAR_WARRIOR, OL_MAHUM_CAPTAIN, OL_MAHUM_GENERAL, TUREK_ORC_ARCHER, TUREK_ORC_SKIRMISHER, TUREK_ORC_SUPPLIER, TUREK_ORC_FOOTMAN, OL_MAHUM_SUPPORT_TROOP);
		SpawnNpcs(OL_MAHUM_SUPPORT_TROOP);
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		if ("DESPAWN".equals(event))
		{
			if (npc != null)
			{
				npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "You should consider going back..."));
				npc.deleteMe();
			}
			...
			switch (npc.getNpcId())
			{
				...
				case TUREK_ORC_ARCHER:
				{
					if (qs.isMemoState(2) && (getQuestItemsCount(killer, BLADE_MOLD) < 20))
					{
						...
						{
							addSpawn(OL_MAHUM_SUPPORT_TROOP, npc.getLocationXYZ(), true, 0, false);
						}
