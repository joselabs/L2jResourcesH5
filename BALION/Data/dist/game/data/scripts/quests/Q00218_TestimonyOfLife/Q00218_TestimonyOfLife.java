...
public final class Q00218_TestimonyOfLife extends Quest
{
	...
	public Q00218_TestimonyOfLife()
	{
		...
		StartNpcs(MASTER_CARDIEN);
		TalkNpcs(MASTER_CARDIEN, HIERARCH_ASTERIOS, BLACKSMITH_PUSHKIN, THALIA, PRIEST_ADONIUS, ARKENIA, ISAEL_SILVERSHADOW);
		KillNpcs(ANT_RECRUIT, ANT_PATROL, ANT_GUARD, ANT_SOLDIER, ANT_WARRIOR_CAPTAIN, HARPY, WYRM, MARSH_SPIDER, GUARDIAN_BASILISK, LETO_LIZARDMAN_SHAMAN, LETO_LIZARDMAN_OVERLORD, UNICORN_OF_EVA);
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		if (qs.isCreated())
		{
			if (npc.getNpcId() == MASTER_CARDIEN)
			{
				if (player.getRace() == Race.Elf)
				{
					if (player.getLevel() >= MIN_LEVEL)
					{
						if ((player.getClassId().getId() == 22) || (player.getClassId().getId() == 19) || (player.getClassId().getId() == 26) || (player.getClassId().getId() == 29))
						{
