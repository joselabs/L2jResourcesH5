...
public final class Q00230_TestOfTheSummoner extends Quest
{
	...
	static
	{
		MONSTERS.put(PAKO_THE_CAT, new MonsterData(CRYSTAL_OF_INPROGRESS_1ST, CRYSTAL_OF_VICTORY_1ST, "Im sorry lord!"));
	...
	public Q00230_TestOfTheSummoner()
	{
		...
		StartNpcs(HIGH_SUMMONER_GALATEA);
		TalkNpcs(HIGH_SUMMONER_GALATEA, GROCER_LARA, SUMMONER_ALMORS, SUMMONER_CAMONIELL, SUMMONER_BELTHUS, SUMMONER_BASILLA, SUMMONER_CELESTIEL, SUMMONER_BRYNTHEA);
		KillNpcs(NOBLE_ANT, NOBLE_ANT_LEADER, WYRM, TYRANT, TYRANT_KINGPIN, BREKA_ORC, BREKA_ORC_ARCHER, BREKA_ORC_SHAMAN, BREKA_ORC_OVERLORD, BREKA_ORC_WARRIOR, FETTERED_SOUL, WINDSUS, GIANT_FUNGUS, MANASHEN_GARGOYLE, LETO_LIZARDMAN, LETO_LIZARDMAN_ARCHER, LETO_LIZARDMAN_SOLDIER, LETO_LIZARDMAN_WARRIOR, LETO_LIZARDMAN_SHAMAN, LETO_LIZARDMAN_OVERLORD, KARUL_BUGBEAR);
		KillNpcs(MONSTERS.keySet());
		AttackNpcs(PAKO_THE_CAT, UNICORN_RACER, SHADOW_TUREN, MIMI_THE_CAT, UNICORN_PHANTASM, SILHOUETTE_TILFO);
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		switch (event)
		{
			...
			case "30635-04.html":
			{
				/// (npc, player, REDUCTION_IN_RECOVERY_TIME, 1000000);
				...
				switch (npc.getScriptValue())
				{
					case 0:
					{
						if (isSummon)
						{
							...
							{
								npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Whhiisshh!"));
					...
					case 1:
					{
						if (!isSummon || (npc.getVariables().getObject("ATTACKER", L2Summon.class) != attacker.getSummon()))
						{
							...
							if (!hasQuestItems(attacker, CRYSTAL_OF_STARTING_1ST) && hasQuestItems(attacker, CRYSTAL_OF_INPROGRESS_1ST) && (qs != null) && qs.isStarted())
							{
								...
								npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "RULE VIOLATION"));
				...
				switch (npc.getScriptValue())
				{
					case 0:
					{
						if (isSummon)
						{
							...
							{
								npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "START DUEL"));
				...
				switch (npc.getScriptValue())
				{
					case 0:
					{
						if (isSummon)
						{
							...
							{
								npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "So shall we start?!"));
				...
				switch (npc.getScriptValue())
				{
					case 0:
					{
						if (isSummon)
						{
							...
							{
								npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Whish! Fight!"));
				...
				switch (npc.getScriptValue())
				{
					case 0:
					{
						if (isSummon)
						{
							...
							{
								npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "I'll walk all over you!"));
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isSummon)
	{
		...
		if ((qs != null) && qs.isStarted() && Util.checkIfInRange(1500, npc, killer, true))
		{
			switch (npc.getNpcId())
			{
				...
				case PAKO_THE_CAT:
				{
					final MonsterData data = MONSTERS.get(npc.getNpcId());
					if (hasQuestItems(killer, data.getCrystalOfInprogress()))
					{
						npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), data.getNpcStringId()));
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		if (qs.isCreated())
		{
			if (npc.getNpcId() == HIGH_SUMMONER_GALATEA)
			{
	...
	private static class MonsterData
	{
		...
		private final String _npcStringId;
		
		protected MonsterData(int crystalOfInprogress, int crystalOfVictory, String npcStringId)
		{
		...
		protected String getNpcStringId()
		{
			return _npcStringId;
		}
	}
