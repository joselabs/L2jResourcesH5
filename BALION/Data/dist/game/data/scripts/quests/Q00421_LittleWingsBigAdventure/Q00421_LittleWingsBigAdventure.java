...
public final class Q00421_LittleWingsBigAdventure extends Quest
{
	...
	public Q00421_LittleWingsBigAdventure()
	{
		...
		StartNpcs(CRONOS);
		TalkNpcs(CRONOS, MIMYU);
		AttackNpcs(TREE_OF_WIND, TREE_OF_STAR, TREE_OF_TWILIGHT, TREE_OF_ABYSS);
		KillNpcs(NPC_DATA.keySet());
	...
	public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isSummon)
	{
		...
		if ((qs != null) && qs.isCond(2))
		{
			if (isSummon)
			{
				final NpcData data = NPC_DATA.get(npc.getNpcId());
				if ((qs.getMemoState() % data.memoStateMod) < data.memoStateValue)
				{
					if (attacker.getSummon().getControlObjectId() == qs.getInt("fluteObjectId"))
					{
						...
						{
							if ((npc.getNpcId() == TREE_OF_ABYSS) && (getRandom(100) < 2))
							{
						...
						else if (getRandom(100) < 2)
						{
							...
							{
								npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Give me a Fairy Leaf"));
				...
				else
				{
					...
					{
						case 0:
							npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Why do you bother me again?"));
							break;
						case 1:
							npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), data.message));
							break;
						case 2:
							npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Leave now, before you incur the wrath of the guardian ghost..."));
							break;
					}
				}
			}
