...
public final class Q00227_TestOfTheReformer extends Quest
{
	...
	public Q00227_TestOfTheReformer()
	{
		...
		StartNpcs(PRIESTESS_PUPINA);
		TalkNpcs(PRIESTESS_PUPINA, PREACHER_SLA, RAMUS, KATARI, KAKAN, NYAKURI, OL_MAHUM_PILGRIM);
		AttackNpcs(NAMELESS_REVENANT, CRIMSON_WEREWOLF);
		KillNpcs(MISERY_SKELETON, SKELETON_ARCHER, SKELETON_MARKSMAN, SKELETON_LORD, SILENT_HORROR, NAMELESS_REVENANT, ARURAUNE, OL_MAHUM_INSPECTOR, OL_MAHUM_BETRAYER, OL_MAHUM_BETRAYER, CRIMSON_WEREWOLF, KRUDEL_LIZARDMAN);
		SpawnNpcs(OL_MAHUM_PILGRIM, OL_MAHUM_INSPECTOR, OL_MAHUM_BETRAYER, CRIMSON_WEREWOLF, KRUDEL_LIZARDMAN);
		...
		if ((qs != null) && qs.isStarted())
		{
			switch (npc.getNpcId())
			{
				...
				case CRIMSON_WEREWOLF:
				{
					...
					{
						npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Cowardly guy!"));
						npc.deleteMe();
					}
		...
		if ((qs != null) && qs.isStarted() && Util.checkIfInRange(1500, npc, killer, true))
		{
			switch (npc.getNpcId())
			{
				...
				case ARURAUNE:
				{
					...
					{
						npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "The concealed truth will always be revealed...!"));
	...
	public String onTalk(L2Npc npc, L2PcInstance player)
	{
		...
		if (qs.isCreated())
		{
			if (npc.getNpcId() == PRIESTESS_PUPINA)
			{
