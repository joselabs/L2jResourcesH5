...
public final class Q00281_HeadForTheHills extends Quest
{
	...
	// Message
	private static final ExShowScreenMessage MESSAGE = new ExShowScreenMessage("Acquisition of SoulShots for Beginners complete, find the Newbie Guide", 5000);
	// Misc
	...
	public static final void giveNewbieReward(L2PcInstance player)
	{
		...
		if ((player.getLevel() < 25) && !vars.getBoolean("NEWBIE_SHOTS", false))
		{
			if (player.isMageClass())
			{
				...
				playSound(player, QuestSound.TUTORIAL_VOICE_027_1000);
			}
			else
			{
				giveItems(player, SOULSHOTS_NO_GRADE_FOR_ROOKIES);
				playSound(player, QuestSound.TUTORIAL_VOICE_026_1000);
			}
