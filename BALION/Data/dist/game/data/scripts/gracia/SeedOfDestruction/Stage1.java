 ...
 */
package gracia.SeedOfDestruction;
...
public class Stage1 extends Quest
{
	...
	private static final String qn = "Stage1";
	...
	private final TIntObjectHashMap<List<SODSpawn>> _spawnList = new TIntObjectHashMap<List<SODSpawn>>();
	...
	private void load()
	{
		...
		try
		{
			...
			File file = new File(Config.DATAPACK_ROOT + "/data/scripts/gracia/SeedOfDestruction/data.xml");
			if (!file.exists())
			{
	...
	public Stage1()
	{
		super(-1, Stage1.class.getSimpleName(), "gracia");
		...
		SpawnNpcs(OBELISK);
		KillNpcs(OBELISK);
		SpawnNpcs(POWERFUL_DEVICE);
		KillNpcs(POWERFUL_DEVICE);
		SpawnNpcs(THRONE_POWERFUL_DEVICE);
		KillNpcs(THRONE_POWERFUL_DEVICE);
		AttackNpcs(TIAT);
		KillNpcs(TIAT);
		KillNpcs(SPAWN_DEVICE);
		SpawnNpcs(TIAT_GUARD);
		KillNpcs(TIAT_GUARD);
		AggroRangeEnterNpcs(TIAT_VIDEO_NPC);
