...
public final class Engine
{
	...
	protected void endGame(L2PcInstance winner)
	{
		...
		final FastMap<Integer, Long> rewards = new FastMap<Integer, Long>();
		for (final String reward : _holder.getString("rewards").split(";"))
		{
			...
			{
				System.out.println("Hide and Seek: Wrong reward format for event npc " + _holder.getInt("npc"));
	...
	protected void spawnRegistrationNpc()
	{
		try
		{
	...
	protected void unspawnRegistrationNpc()
	{
		_regNpc.stopRespawn();
	...
	protected void spawnHideNpc()
	{
		try
		{
			_hideNpc = new L2Spawn(NpcTable.getInstance().getTemplate(_holder.getInt("npc")));
			_hideNpc.setLocx(_holder.getInt("x"));
			_hideNpc.setLocy(_holder.getInt("y"));
			_hideNpc.setLocz(_holder.getInt("z"));
	...
	protected class Registration implements Runnable
	{
		...
		public void run()
		{
			...
			final int mins = Config.HAS_REG_MINS_DURATION;
			...
			else
			{
				...
				final long end = Config.HAS_EVENT_MINS_DURATION * 60000;
	...
	protected class Game implements Runnable
	{
		@Override
		public void run()
		{
	...
	protected class ClueTask implements Runnable
	{
		@Override
		public void run()
		{
