...
public class Slaves extends L2AttackableAIScript
{
		...
		if (((L2MonsterInstance) npc).getMinionList() != null)
		{
			...
			if ((slaves != null) && !slaves.isEmpty())
			{
				for (L2MonsterInstance slave : slaves)
				{
					...
					slave.abortAttack();
					slave.abortCast();
					slave.broadcastPacket(new CreatureSay(slave.getObjectId(), 1, slave.getName(), "Thank you for saving me from the clutches of evil!"));
	...
	public Slaves()
	{
		super(-1, Slaves.class.getSimpleName(), "hellbound");
		SpawnNpcs(MASTERS);
		KillNpcs(MASTERS);
	}
}
