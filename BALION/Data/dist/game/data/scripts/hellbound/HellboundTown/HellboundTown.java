...
public class HellboundTown extends Quest
{
	...
	protected static final int[] ENTRY_POINT =
	{
		14117,
		255434,
		-2016
	...
	{
		super(-1, HellboundTown.class.getSimpleName(), "hellbound");
		FirstTalkNpcs(DOWNTOWN_NATIVE);
		StartNpcs(KANAF, DOWNTOWN_NATIVE);
		TalkNpcs(KANAF, DOWNTOWN_NATIVE);
		AttackNpcs(TOWN_GUARD, KEYMASTER);
		AggroRangeEnterNpcs(TOWN_GUARD);
		KillNpcs(AMASKARI);
		SpawnNpcs(DOWNTOWN_NATIVE, TOWN_GUARD, TOWN_PATROL, KEYMASTER);
	}
		...
		if ((tmpworld != null) && (tmpworld instanceof TownWorld))
		{
			...
			if (npc.getNpcId() == DOWNTOWN_NATIVE)
			{
				...
				else if (event.equalsIgnoreCase("break_chains"))
				{
					...
					{
						npc.broadcastPacket(new CreatureSay(npc.getObjectId(), Say2.ALL, npc.getName(), NATIVES_NPCSTRING_ID[0]));
						npc.broadcastPacket(new CreatureSay(npc.getObjectId(), Say2.ALL, npc.getName(), NATIVES_NPCSTRING_ID[2]));
					}
					else
					{
						...
						npc.broadcastPacket(new CreatureSay(npc.getObjectId(), Say2.ALL, npc.getName(), NATIVES_NPCSTRING_ID[0]));
						npc.broadcastPacket(new CreatureSay(npc.getObjectId(), Say2.ALL, npc.getName(), NATIVES_NPCSTRING_ID[1]));
	...
	public String onAggroRangeEnter(L2Npc npc, L2PcInstance player, boolean isPet)
	{
		...
		if ((tmpworld != null) && (tmpworld instanceof TownWorld))
		{
			...
			if (!npc.isBusy())
			{
				npc.broadcastPacket(new CreatureSay(npc.getObjectId(), Say2.ALL, npc.getName(), NPCSTRING_ID[0]));
				npc.setBusy(true);
	...
	public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isPet, L2Skill skill)
	{
		...
		if ((tmpworld != null) && (tmpworld instanceof TownWorld))
		{
			...
			if (!world.isAmaskariDead && !(npc.getBusyMessage().equalsIgnoreCase("atk") || npc.isBusy()))
			{
				...
				if (msgId >= 0)
				{
					npc.broadcastPacket(new CreatureSay(npc.getObjectId(), Say2.ALL, npc.getName(), NPCSTRING_ID[msgId]));
				}
				npc.setBusy(true);
			...
			if ((_caller != null) && !_caller.isDead())
			{
				...
				if ((tmpworld != null) && (tmpworld instanceof TownWorld))
				{
					...
					if ((world.spawnedAmaskari != null) && !world.spawnedAmaskari.isDead())
					{
						...
						world.spawnedAmaskari.broadcastPacket(new CreatureSay(world.spawnedAmaskari.getObjectId(), Say2.ALL, world.spawnedAmaskari.getName(), "I'll make you feel suffering like a flame that is never extinguished!"));
					}
				}
			}
		}
