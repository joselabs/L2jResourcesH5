...
public class Quarry extends Quest
{
			...
			if (npc.getNpcId() == SLAVE)
			{
				if (!npc.isDead() && !npc.isDecayed() && (npc.getAI().getIntention() == CtrlIntention.AI_INTENTION_FOLLOW))
				{
					if (HellboundManager.getInstance().getLevel() == 5)
					{
						...
						try
						{
							npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Thank you for the rescue. It's a small gift."));
						}
						catch (Exception e)
						{
							//
	...
	public Quarry()
	{
		super(-1, Quarry.class.getSimpleName(), "hellbound");
		SpawnNpcs(SLAVE);
		FirstTalkNpcs(SLAVE);
		StartNpcs(SLAVE);
		TalkNpcs(SLAVE);
