...
public class TullyWorkshop extends Quest
{
	...
	protected int nextServantIdx = 0;
	protected int killedFollowersCount = 0;
	protected boolean allowServantSpawn = true;
	protected boolean allowAgentSpawn = true;
	protected boolean allowAgentSpawn_7th = true;
	protected boolean is7thFloorAttackBegan = false;
	...
	{
		super(-1, TullyWorkshop.class.getSimpleName(), "hellbound");
		...
		StartNpcs(AGENT);
		StartNpcs(CUBE_68);
		StartNpcs(INGENIOUS_CONTRAPTION);
		StartNpcs(DWARVEN_GHOST);
		StartNpcs(TOMBSTONE);
		TalkNpcs(AGENT);
		TalkNpcs(CUBE_68);
		TalkNpcs(INGENIOUS_CONTRAPTION);
		TalkNpcs(DWARVEN_GHOST);
		TalkNpcs(DWARVEN_GHOST);
		TalkNpcs(TOMBSTONE);
		FirstTalkNpcs(AGENT);
		FirstTalkNpcs(CUBE_68);
		FirstTalkNpcs(INGENIOUS_CONTRAPTION);
		FirstTalkNpcs(DWARVEN_GHOST);
		FirstTalkNpcs(TOMBSTONE);
		KillNpcs(TULLY);
		KillNpcs(TIMETWISTER_GOLEM);
		KillNpcs(TEMENIR);
		KillNpcs(DRAXIUS);
		KillNpcs(KIRETCENAH);
		KillNpcs(DARION);
		KillNpcs(PILLAR);
		FactionCallNpcs(TEMENIR);
		FactionCallNpcs(DRAXIUS);
		FactionCallNpcs(KIRETCENAH);
		
		SpawnNpcs(CUBE_68);
		SpawnNpcs(DARION);
		SpawnNpcs(TULLY);
		SpawnNpcs(PILLAR);
		SpellFinishedNpcs(AGENT);
		SpellFinishedNpcs(TEMENIR);
	...
	public final String onFirstTalk(L2Npc npc, L2PcInstance player)
	{
		...
		else if (npcId == DWARVEN_GHOST)
		{
			...
			{
				npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "HA-HA! You were so afraid of death... let me see... If you find me in time... maybe you can... find a way ..."));
				npc.deleteMe();
				return null;
			}
	...
	public final String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		{
			npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "De-activate the alarm."));
	...
	public String onKill(L2Npc npc, L2PcInstance killer, boolean isPet)
	{
		...
		if ((npcId == TULLY) && npc.isInsideRadius(-12557, 273901, -9000, 1000, false, false))
		{
			...
			postMortemSpawn.get(0).sendPacket(new CreatureSay(postMortemSpawn.get(0).getObjectId(), 1, postMortemSpawn.get(0).getName(), "Detonator initialization - time set for " + Integer.toString((countdownTime / 60000)) + " minute(s) from now"));
		}
		else if ((npcId == TIMETWISTER_GOLEM) && (_countdown != null))
		{
			if (Rnd.get(1000) >= 700)
			{
				npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "A fatal error has occurred."));
				if (countdownTime > 180000)
				{
					...
					{
						postMortemSpawn.get(0).sendPacket(new CreatureSay(postMortemSpawn.get(0).getObjectId(), 1, postMortemSpawn.get(0).getName(), "Zzzz- city interference error - forward effect created"));
					}
				}
			}
			else
			{
				npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Time rift device activation successful!"));
				if ((countdownTime > 0) && (countdownTime <= 420000))
				{
					countdownTime += 180000;
					if ((postMortemSpawn != null) && (postMortemSpawn.size() > 0) && (postMortemSpawn.get(0) != null) && (postMortemSpawn.get(0).getNpcId() == INGENIOUS_CONTRAPTION))
					{
						postMortemSpawn.get(0).sendPacket(new CreatureSay(postMortemSpawn.get(0).getObjectId(), 1, postMortemSpawn.get(0).getName(), "Zzzz- city interference error - recurrence effect created-"));
					}
				}
			}
		}
		...
		else if ((npcId >= SERVANT_FIRST) && (npcId <= SERVANT_LAST))
		{
			...
			{
				npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "I failed... Please forgive me, Darion..."));
			}
			else
			{
				npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), killer.getName() + ", I'll be back... don't get comfortable..."));
			}
		}
	...
	public String onSpellFinished(L2Npc npc, L2PcInstance player, L2Skill skill)
	{
		...
		{
			npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), player.getName() + ", thank you... for giving me your life..."));
		...
		public void run()
		{
			...
			if (countdownTime > 60000)
			{
				if ((countdownTime % 60000) == 0)
				{
					...
					{
						npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), Integer.toString((countdownTime / 60000)) + " minute(s) remaining."));
					}
				}
			}
			...
			else
			{
				if ((npc != null) && (npc.getNpcId() == INGENIOUS_CONTRAPTION))
				{
					npc.sendPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), Integer.toString((countdownTime / 1000)) + " second(s) remaining."));
				}
			}
		}
	}
