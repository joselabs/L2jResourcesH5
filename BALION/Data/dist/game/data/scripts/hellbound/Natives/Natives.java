...
public class Natives extends Quest
{
	...
	public final String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		if (npc.getNpcId() == TRAITOR)
		{
			if (event.equalsIgnoreCase("open_door"))
			{
				...
				if (qs.getQuestItemsCount(MARK_OF_BETRAYAL) >= 10)
				{
					...
					npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Alright, now Leodas is yours!"));
		...
		{
			npc.broadcastPacket(new CreatureSay(npc.getObjectId(), 1, npc.getName(), "Hun.. hungry"));
			npc.doDie(null);
		}
	...
	public Natives()
	{
		super(-1, Natives.class.getSimpleName(), "hellbound");
		FirstTalkNpcs(NATIVE, INSURGENT, INCASTLE);
		StartNpcs(TRAITOR, INCASTLE);
		TalkNpcs(TRAITOR, INCASTLE);
		SpawnNpcs(NATIVE);
	}
}
