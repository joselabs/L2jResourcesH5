...
public class TowerOfNaia extends Quest
{
	...
	public TowerOfNaia()
	{
		super(-1, TowerOfNaia.class.getSimpleName(), "hellbound");
		
		FirstTalkNpcs(CONTROLLER);
		StartNpcs(CONTROLLER);
		StartNpcs(DWARVEN_GHOST);
		TalkNpcs(CONTROLLER);
		TalkNpcs(DWARVEN_GHOST);
		AttackNpcs(LOCK);
		KillNpcs(LOCK);
		KillNpcs(MUTATED_ELPY);
		SpawnNpcs(MUTATED_ELPY);
		KillNpcs(SPORE_BASIC);
		SpawnNpcs(SPORE_BASIC);
	...
	public final String onFirstTalk(L2Npc npc, L2PcInstance player)
	{
		...
		else if ((npcId >= ROOM_MANAGER_FIRST) && (npcId <= ROOM_MANAGER_LAST))
		{
			if (_activeRooms.containsKey(npcId) && !_activeRooms.get(npcId))
			{
				...
				{
					player.sendMessage("You can operate with machine only when you are in a party.");
					return null;
				}
				return "manager.htm";
			}
	...
	public String onAttack(L2Npc npc, L2PcInstance attacker, int damage, boolean isSummon, L2Skill skill)
	{
		if ((_lock != null) && (npc.getObjectId() == _lock.getObjectId()))
		{
			...
			if ((remaindedHpPercent <= _counter) && (_controller != null))
			{
				...
				_controller.broadcastPacket(new CreatureSay(_controller.getObjectId(), 1, _controller.getName(), "Emergency! Emergency! The outer wall is weakening rapidly!"));
				_counter -= 10;
			}
		}
				...
				if (sporeGroup >= 0)
				{
					...
					if ((Math.abs(_indexCount[sporeGroup]) < ELEMENT_INDEX_LIMIT) && (Math.abs(_indexCount[sporeGroup]) > 0) && ((_indexCount[sporeGroup] % 20) == 0) && (getRandom(100) < 50))
					{
						for (L2Npc spore : _sporeSpawn)
						{
							...
							{
								spore.broadcastPacket(new CreatureSay(spore.getObjectId(), 1, spore.getName(), SPORES_NPCSTRING_ID[getRandom(4)]));
							}
						}
					}
	...
	public final String onSpawn(L2Npc npc)
	{
		...
		else if (((npcId == SPORE_BASIC) || ((npcId >= SPORE_FIRE) && (npcId <= SPORE_EARTH))) && (_challengeState == STATE_SPORE_CHALLENGE_IN_PROGRESS))
		{
			...
			npc.getSpawn().setXLocation(coord[0]);
			npc.getSpawn().setYLocation(coord[1]);
			npc.getSpawn().setZLocation(coord[2]);
	...
	private int moveTo(L2Npc npc, int[] coords)
	{
		...
		if (npc != null)
		{
			...
			npc.getSpawn().setXLocation(coords[0]);
			npc.getSpawn().setYLocation(coords[1]);
			npc.getSpawn().setZLocation(coords[2]);
		}
		...
		if ((party != null) && ZONES.containsKey(managerId) && (ZoneManager.getInstance().getZoneById(ZONES.get(managerId)) != null))
		{
			...
			for (L2PcInstance player : zone.getPlayersInside())
			{
				if (player != null)
				{
					...
					if ((charParty == null) || (charParty.getLeader() != party.getLeader()))
					{
