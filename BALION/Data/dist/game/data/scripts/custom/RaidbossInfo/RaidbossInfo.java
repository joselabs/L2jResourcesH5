 ...
 */
public final class RaidbossInfo extends L2AttackableAIScript
{
	// @formatter:off
	...
	public RaidbossInfo()
	{
		...
		StartNpcs(NPC);
		TalkNpcs(NPC);
		RAIDS.addAll(NpcTable.getInstance().getByFilter(npc -> npc.type.equalsIgnoreCase("l2raidboss")).stream().map(npc -> npc.getNpcId()).collect(Collectors.toList()));
		// _log.info("raid:" + RAIDS.toString());
	}
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		if (Util.isDigit(event))
		{
			...
			if (RAIDS.contains(bossId))
			{
				...
				if (spawn != null)
				{
					final Location loc = spawn.getSpawnLocation();
