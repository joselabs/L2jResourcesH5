...
public final class SkillTransferValidator extends Quest
{
	...
	{
		super(-1, SkillTransferValidator.class.getSimpleName(), "custom");
	...
	private void givePormanders(L2PcInstance player)
	{
		...
		if (index >= 0)
		{
			QuestState st = player.getQuestState(SkillTransferValidator.class.getSimpleName());
			if (st == null)
			{
				st = newQuestState(player);
			}
			
			final String name = SkillTransferValidator.class.getSimpleName() + String.valueOf(player.getClassId().getId());
			if (st.getInt(name) == 0)
			{
				...
				if (st.getGlobalQuestVar(name).isEmpty())
				{
					...
					player.addItem(SkillTransferValidator.class.getSimpleName(), PORMANDERS[index][0], PORMANDERS[index][1], null, true);
				}
			}
