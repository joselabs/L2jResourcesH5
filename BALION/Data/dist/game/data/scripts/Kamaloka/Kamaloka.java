...
public class Kamaloka extends Quest
{
	 ...
	 */
	private static final boolean STEALTH_SHAMAN = false;
	...
	public final String onKill(L2Npc npc, L2PcInstance player, boolean isPet)
	{
		...
		if (tmpWorld instanceof KamaWorld)
		{
			...
			if ((world.boss != null) && (world.boss == npc))
			{
				...
				if (player.isGM())
				{
					player.sendPacket(new ExShowScreenMessage("Gm, you can leave instance, thx for testing me",5000));
					// destroy instance after EXIT_TIME
					Instance inst = InstanceManager.getInstance().getInstance(world.instanceId);
					inst.setDuration(EXIT_TIME * 1000); // destroy time 5 sec
					inst.setEmptyDestroyTime(0);
				}
				else
				{
	...
	public Kamaloka()
	{
		super(-1, Kamaloka.class.getSimpleName(), "");
		FirstTalkNpcs(TELEPORTER);
		TalkNpcs(TELEPORTER);
