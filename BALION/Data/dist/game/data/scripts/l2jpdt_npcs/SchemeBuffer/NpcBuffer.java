package l2jpdt_npcs.SchemeBuffer;
...
public class NpcBuffer extends Quest
{
	...
	{
		_log.warning("L2jPDT Scheme Buffer:" + e.toString() + "");
		if (DEBUG)
		{
			e.printStackTrace();
		}
	...
	private static final boolean ENABLE_SCHEME_BUFFER = Config.ENABLE_SCHEME_BUFFER;
	private static final int NPC_ID = Config.NpcBuffer_NPC_ID;
	
	private static final String TITLE_NAME = "Scheme Buffer";
	private static final boolean SCRIPT_RELOAD = Config.NpcBuffer_CanReload;
	...
	private static final int MAX_SCHEME_BUFFS = Config.BuffMaxAmount;
	private static final int MAX_SCHEME_DANCES = Config.DanceMaxAmout;
	...
	public NpcBuffer()
	{
		super(NPC_ID, NpcBuffer.class.getSimpleName(), "l2jpdt_npcs");
		StartNpcs(NPC_ID);
		FirstTalkNpcs(NPC_ID);
		TalkNpcs(NPC_ID);
		if (ENABLE_SCHEME_BUFFER)
		{
			_log.info("L2jPDT: Scheme Buffer NPC is enabled, NPC loaded.");
		}
		else
		{
			_log.info("L2jPDT: Scheme Buffer NPC is disabled, NPC will not work.");
		}
	}
	...
	{
		// TRACE("onAdvEvent('"+event+"',"+(npc == null ? "NULL" : npc.getNpcId()+npc.getName())+","+(player == null ? "NULL" : player.getName())+")");/*@IF DEBUG@*/
		if (DEBUG)
		{
			System.out.println(getScriptName() + "#onAdvEvent('" + event + "'," + (npc == null ? "NULL" : npc.getNpcId() + npc.getName()) + "," + (player == null ? "NULL" : player.getName()) + ")");
		}
				...
				if ((int) (System.currentTimeMillis() / 1000) > st.getInt("blockUntilTime"))
				{
					...
					for (int i = 0; i < buffs.size(); ++i)
					{
						...
						else
						{
							if (player.getSummon() != null)
							{
								SkillTable.getInstance().getInfo(buffs.get(i), levels.get(i)).getEffects(player.getSummon(), player.getSummon());
							}
							else
							{
								return showText(st, "Info", "You can't use the Pet's options.<br>Summon your pet first!", false, "Return", "main");
				...
				if ((int) (System.currentTimeMillis() / 1000) > st.getInt("blockUntilTime"))
				{
					...
					else
					{
						...
						else
						{
							if (player.getSummon() != null)
							{
								SkillTable.getInstance().getInfo(Integer.parseInt(eventParam1), Integer.parseInt(eventParam2)).getEffects(player.getSummon(), player.getSummon());
							}
							else
							{
								return showText(st, "Info", "You can't use the Pet's options.<br>Summon your pet first!", false, "Return", "main");
				...
				if ((int) (System.currentTimeMillis() / 1000) > st.getInt("blockUntilTime"))
				{
					...
					else
					{
						if (player.getSummon() != null)
						{
							...
							for (int[] i : buff_sets)
							{
								SkillTable.getInstance().getInfo(i[0], i[1]).getEffects(player.getSummon(), player.getSummon());
							}
						}
						else
						{
	...
	public String onFirstTalk(L2Npc npc, L2PcInstance player)
	{
		...
		if (!ENABLE_SCHEME_BUFFER)
		{
			return showText(st, "Info", "Buffer is <font color=FF0000>not allowed!</font><br>Contact admin for enable it.", false, "Return", "main");
		}
		if (player.isGM())
		{
			if (SCRIPT_RELOAD)
		...
		else if (TvTRoundEvent.isPlayerParticipant(player.getObjectId()))
		{
			return showText(st, "Info", "You can not buff while you are in <font color=\"FF0000\">TvT Round!</font><br>Come back,<br>when you are out of TvT Round!", false, "Return", "main");
		}
