...
import l2r.gameserver.model.holders.ClientHardwareInfoHolder;
...
import gr.sr.network.GameClientState;
...
public class L2GameClient extends MMOClient<MMOConnection<L2GameClient>>
{
			...
			try
			{
				if ((getActiveChar() != null) && !isDetached())
				{
					...
					if (offlineMode(getActiveChar()))
					{
						...
						if (OfflineTradeConfigs.OFFLINE_SET_NAME_COLOR)
						{
							getActiveChar().getAppearance().setNameColor(OfflineTradeConfigs.OFFLINE_NAME_COLOR);
	...
	protected boolean offlineMode(L2PcInstance player)
	{
		...
		switch (player.getPrivateStoreType())
		{
			...
			case BUY:
			{
				canSetShop = OfflineTradeConfigs.OFFLINE_TRADE_ENABLE;
				break;
			}
			case MANUFACTURE:
			{
			...
			default:
			{
				canSetShop = OfflineTradeConfigs.OFFLINE_CRAFT_ENABLE && player.isInCraftMode();
				break;
			}
		}
		
		if (OfflineTradeConfigs.OFFLINE_MODE_IN_PEACE_ZONE && !player.isInsideZone(ZoneIdType.PEACE))
		{
			canSetShop = false;
		}
		return canSetShop;
