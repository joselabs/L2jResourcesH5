...
public class L2PlayerAI extends L2PlayableAI
{
	...
	protected void onIntentionMoveTo(Location loc)
	{
		...
		switch (_actor.getActingPlayer().getDuelState())
		{
			case DEAD:
				clientActionFailed();
	...
	private void thinkCast()
	{
		...
		else
		{
			...
			{
				if (_skill.isOffensive() && (getInteractionTarget() != null))
				{
					// Notify the target
					setInteractionTarget(null);
				}
		...
		if ((oldTarget != null) && (target != null) && (oldTarget != target))
		{
			...
			_actor.getActingPlayer().setTargetWithoutUpdates(getInteractionTarget());
