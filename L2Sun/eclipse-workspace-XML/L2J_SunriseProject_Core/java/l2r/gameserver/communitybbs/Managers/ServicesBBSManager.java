...
public class ServicesBBSManager extends BaseBBSManager
{
		...
		else if (command.startsWith(_servicesBBSCommand + "_teleport"))
		{
			...
			try
			{
				...
				if (Conditions.checkPlayerItemCount(activeChar, itemIdToGet, price))
				{
					...
					{
						if (CommunityServicesConfigs.TELEPORT_DELAY > 0)
						{
							doTeleportLong(activeChar, c[0], c[1], c[2]);
						}
						else
						{
							activeChar.teleToLocation(c[0], c[1], c[2]);
	...
	public void doTeleportLong(L2PcInstance player, int x, int y, int z)
	{
		player.abortCast();
		player.abortAttack();
		...
		final int tpdelay = CommunityServicesConfigs.TELEPORT_DELAY * 1000;
		
		Broadcast.toSelfAndKnownPlayers(player, new MagicSkillUse(player, 1050, 1, tpdelay, 0));
		player.sendPacket(new SetupGauge(SetupGauge.BLUE, tpdelay));
		player.setSkillCast(ThreadPoolManager.getInstance().scheduleGeneral(new Teleport(player, x, y, z), tpdelay));
		player.forceIsCasting(10 + GameTimeController.getInstance().getGameTicks() + (tpdelay / GameTimeController.MILLIS_IN_TICK));
	}
