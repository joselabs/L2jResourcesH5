...
public abstract class L2Summon extends L2Playable
{
	...
	public void sendDamageMessage(L2Character target, int damage, boolean mcrit, boolean pcrit, boolean miss)
	{
		...
		if (target.getObjectId() != getOwner().getObjectId())
		{
			...
			OlympiadGameManager.getInstance().notifyCompetitorDamage(getOwner(), target, damage);
	...
	public void updateAndBroadcastStatus(BroadcastStatusType val)
	{
		...
		_updateAndBroadcastStatus = ThreadPoolManager.getInstance().scheduleGeneral(() ->
		{
			sendPacket(new PetInfo(this, val.get()));
			sendPacket(new PetStatusUpdate(this));
			if (isVisible())
			{
				broadcastNpcInfo(val.get(), false);
			}
			L2Party party = getOwner().getParty();
			if (party != null)
			{
		...
		} , Config.stats_update_packetsDelay);
	}
	
	public void broadcastNpcInfo(int val)
	{
		broadcastNpcInfo(val, false);
	}
	
	public void broadcastNpcInfo(int val, boolean force)
	{
		try
		{
			if ((Config.npcInfo_packetsDelay == 0) || force)
			{
				if (_npcInfoTask != null)
				{
					_npcInfoTask.cancel(false);
					_npcInfoTask = null;
				}
				
				broadcastCharInfoImpl(val);
				return;
			}
			...
			_npcInfoTask = ThreadPoolManager.getInstance().scheduleGeneral(() ->
			{
				broadcastCharInfoImpl(val);
			} , Config.npcInfo_packetsDelay);
		}
		catch (Exception e)
		{
	...
	public void broadcastCharInfoImpl(int val)
	{
