...
import gr.sr.advancedBuffer.PlayerScheme;
import gr.sr.advancedBuffer.SchemeBufferBBSManager;
import gr.sr.autoFarm.AutoFarmerData;
import gr.sr.autoFarm.AutofarmManager;
...
public final class L2PcInstance extends L2Playable
{
	...
	public int getRelation(L2PcInstance target)
	{
		...
		{
			if ((target.getPledgeType() != ClanUnitTypes.SUBUNIT_ACADEMY.getId()) && (getPledgeType() != ClanUnitTypes.SUBUNIT_ACADEMY.getId()) && target.getClan().isAtWarWith(getClan().getId()))
			{
	...
	{
		// Stop farmer is pvp flag is greater than zero
		if (pvpFlag != 0)
		{
			AutofarmManager.INSTANCE.stopFarm(getFarmerData());
		}
	...
	public void useEquippableItem(L2ItemInstance item, boolean abortAttack)
	{
		...
		// Need force update in order to view inventory properly
	...
	public void onKillUpdatePvPKarma(L2Character target)
	{
		...
		else
		{
			...
			// check about wars
			if ((targetPlayer.getClan() != null) && (getClan() != null) && getClan().isAtWarWith(targetPlayer.getClanId()) && targetPlayer.getClan().isAtWarWith(getClanId()) && (targetPlayer.getPledgeType() != ClanUnitTypes.SUBUNIT_ACADEMY.getId()) && (getPledgeType() != ClanUnitTypes.SUBUNIT_ACADEMY.getId()))
			{
	...
	{
		if (isInsideZone(ZoneIdType.PVP) || (getKarma() > 0))
		{
			return;
		}
	...
	public void updatePvPStatus(L2Character target)
	{
		...
		if ((!isInsideZone(ZoneIdType.PVP) || !player_target.isInsideZone(ZoneIdType.PVP)) && (getKarma() == 0) && (player_target.getKarma() == 0))
		{
	...
	public void setPrivateStoreType(PrivateStoreType privateStoreType)
	{
		...
		if (OfflineTradeConfigs.OFFLINE_DISCONNECT_FINISHED && (privateStoreType == PrivateStoreType.NONE) && ((getClient() == null) || getClient().isDetached()))
		{
			logout();
		}
	}
	...
	private static L2PcInstance restore(int objectId)
	{
			...
			PreparedStatement statement = con.prepareStatement(RESTORE_CHARACTER))
		{
			...
			SchemeBufferBBSManager.loadSchemes(player, con);
	...
	public final void sendDamageMessage(L2Character target, int damage, boolean mcrit, boolean pcrit, boolean miss)
	{
		...
		OlympiadGameManager.getInstance().notifyCompetitorDamage(this, target, damage);
	...
	{
		if (!Config.ENABLE_FALLING_DAMAGE || isDead() || isFlying() || isFlyingMounted() || isInWater() || isInBoat())
		{
			return;
		}
	...
	{
		if (!MultilingualConfigs.MULTILANG_ENABLE)
		{
			return null;
		}
	...
	public boolean setLang(String lang)
	{
		...
		if (MultilingualConfigs.MULTILANG_ENABLE)
		{
			if (MultilingualConfigs.MULTILANG_ALLOWED.contains(lang))
			{
				_lang = lang;
				result = true;
			}
			...
			{
				_lang = MultilingualConfigs.MULTILANG_DEFAULT;
			}
	...
	{
		return (!isInStoreMode() && (getActiveRequester() == null) && !isAlikeDead() && !isAllSkillsDisabled() && !isCastingNow() && !isCastingSimultaneouslyNow() && ((getAI().getIntention() == CtrlIntention.AI_INTENTION_IDLE) || (getAI().getIntention() == CtrlIntention.AI_INTENTION_ACTIVE)));
	}
	...
	private AutoFarmerData _farmerData;
	
	public AutoFarmerData getFarmerData()
	{
		if (_farmerData == null)
		{
			_farmerData = new AutoFarmerData(this);
		}
		return _farmerData;
	}
	
	public boolean hasAutofarmService()
	{
		return getQuickVarL(QuickVarType.AUTO_FARMER.getCommand(), 0) > System.currentTimeMillis();
	}
	
	private final List<PlayerScheme> buffSchemes = new CopyOnWriteArrayList<>();
	...
	{
		for (PlayerScheme scheme : buffSchemes)
		{
	...
	{
		return isInEvents() || isInOfflineMode() || isInOlympiad() || isInOlympiadMode() || OlympiadManager.getInstance().isRegistered(this) || OlympiadManager.getInstance().isRegisteredInComp(this);
	}
	
	private AbstractMap.SimpleEntry<Integer, OnAnswerListener> _askDialog = null;
	...
	public void ask(ConfirmDlg dlg, OnAnswerListener listener)
	{
		...
		int rnd = Rnd.nextInt();
		_askDialog = new AbstractMap.SimpleEntry<>(rnd, listener);
		dlg.addRequesterId(rnd);
		sendPacket(dlg);
	}
	
	public AbstractMap.SimpleEntry<Integer, OnAnswerListener> getAskListener(boolean clear)
	{
		if (!clear)
		{
			return _askDialog;
		...
		final AbstractMap.SimpleEntry<Integer, OnAnswerListener> ask = _askDialog;
		_askDialog = null;
		return ask;
	}
