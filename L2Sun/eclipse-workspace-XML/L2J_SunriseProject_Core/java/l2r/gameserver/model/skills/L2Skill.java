...
public abstract class L2Skill implements IChanceSkillTrigger, IIdentifiable
{
	...
	{
		final int minAffect = _affectLimit[0] == 0 ? 99 : _affectLimit[0];
		final int randomAffect = _affectLimit[1] == 0 ? 99 : _affectLimit[1];
		final boolean hasLimit = _affectLimit[2] == 1;
		final int random = minAffect + Rnd.get(randomAffect);
		
		return hasLimit ? Math.min(random, 10) : random;
	}
	...
	{
		if ((target == null) || (caster == null) || target.isDead() || (target == caster))
		{
			return false;
		}
