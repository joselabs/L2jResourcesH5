...
import l2r.gameserver.enums.AllyPenaltyTypes;
...
public class L2Clan implements IIdentifiable, INamable
{
	...
	public int getMaxNrOfMembers(int pledgeType)
	{
		switch (pledgeType)
		{
			...
			default:
				return getSubPledge(pledgeType).getSize();
		}
	}
	...
	public static class SubPledge
	{
		...
		public SubPledge(int id, String name, int leaderId)
		{
			...
			if ((id == ClanUnitTypes.SUBUNIT_ACADEMY.getId()) || (id < ClanUnitTypes.SUBUNIT_KNIGHT1.getId()))
			{
				_size = 20;
			}
			else if (id > ClanUnitTypes.SUBUNIT_ROYAL2.getId())
			{
				_size = 10;
			}
		}
		...
		public void setAsReinforced()
		{
			if (_id == ClanUnitTypes.SUBUNIT_ACADEMY.getId())
			{
				return;
			}
			if (_id < ClanUnitTypes.SUBUNIT_KNIGHT1.getId())
			{
				_size = 30;
			}
			else
	...
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("SELECT sub_pledge_id,name,leader_id,reinforced FROM clan_subpledges WHERE clan_id=?"))
		{
			...
			try (ResultSet rset = statement.executeQuery())
			{
				while (rset.next())
				{
					...
					boolean reinforced = rset.getInt("reinforced") == 1;
					// Create a SubPledge object for each record
					SubPledge pledge = new SubPledge(id, name, leaderId);
					if (reinforced)
					{
						pledge.setAsReinforced();
					}
	...
	public SubPledge createSubPledge(L2PcInstance player, int pledgeType, int leaderId, String subPledgeName)
	{
		...
		if ((pledgeType != -1) && (((getReputationScore() < Config.ROYAL_GUARD_COST) && (pledgeType < ClanUnitTypes.SUBUNIT_KNIGHT1.getId())) || ((getReputationScore() < Config.KNIGHT_UNIT_COST) && (pledgeType > ClanUnitTypes.SUBUNIT_ROYAL2.getId()))))
		{
	...
	public int getAvailablePledgeTypes(int pledgeType)
	{
		...
		{
			final ClanUnitTypes unit = ClanUnitTypes.getById(pledgeType);
			switch (unit)
			{
				case SUBUNIT_ACADEMY:
					return 0;
				case SUBUNIT_ROYAL1:
					pledgeType = getAvailablePledgeTypes(ClanUnitTypes.SUBUNIT_ROYAL2.getId());
					break;
				case SUBUNIT_ROYAL2:
					return 0;
				case SUBUNIT_KNIGHT1:
					pledgeType = getAvailablePledgeTypes(ClanUnitTypes.SUBUNIT_KNIGHT2.getId());
					break;
				case SUBUNIT_KNIGHT2:
					pledgeType = getAvailablePledgeTypes(ClanUnitTypes.SUBUNIT_KNIGHT3.getId());
					break;
				case SUBUNIT_KNIGHT3:
					pledgeType = getAvailablePledgeTypes(ClanUnitTypes.SUBUNIT_KNIGHT4.getId());
					break;
	...
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();
			PreparedStatement statement = con.prepareStatement("UPDATE clan_subpledges SET leader_id=?, name=?, reinforced=? WHERE clan_id=? AND sub_pledge_id=?"))
		{
			statement.setInt(1, getSubPledge(pledgeType).getLeaderId());
			statement.setString(2, getSubPledge(pledgeType).getName());
			statement.setInt(3, getSubPledge(pledgeType).getSize() > 20 ? 1 : 0);
			statement.setInt(4, getId());
			statement.setInt(5, pledgeType);
			statement.execute();
			if (Config.DEBUG)
			{
	...
	public boolean checkAllyJoinCondition(L2PcInstance activeChar, L2PcInstance target)
	{
		...
		{
			if (leaderClan.getAllyPenaltyType() == AllyPenaltyTypes.PENALTY_TYPE_DISMISS_CLAN.getId())
			{
		...
		{
			if (targetClan.getAllyPenaltyType() == AllyPenaltyTypes.PENALTY_TYPE_CLAN_LEAVED.getId())
			{
			...
			if (targetClan.getAllyPenaltyType() == AllyPenaltyTypes.PENALTY_TYPE_CLAN_DISMISSED.getId())
			{
	...
	public void createAlly(L2PcInstance player, String allyName)
	{
		...
		if (getAllyPenaltyExpiryTime() > System.currentTimeMillis())
		{
			if (getAllyPenaltyType() == AllyPenaltyTypes.PENALTY_TYPE_DISSOLVE_ALLY.getId())
			{
	...
	public void dissolveAlly(L2PcInstance player)
	{
		...
		setAllyPenaltyExpiryTime(currentTime + (Config.ALT_CREATE_ALLY_DAYS_WHEN_DISSOLVED * 86400000L), AllyPenaltyTypes.PENALTY_TYPE_DISSOLVE_ALLY.getId()); // 24*60*60*1000 = 86400000
		updateClanInDB();
	}
	...
	public int getTerritoryIdLord()
	{
		if (getCastleId() > 0)
		{
			if ((TerritoryWarManager.getInstance().getTerritory(getCastleId()) != null) && (TerritoryWarManager.getInstance().getTerritory(getCastleId()).getLordId() == getLeaderId()))
			{
				return TerritoryWarManager.getInstance().getTerritory(getCastleId()).getTerritoryId();
			}
		}
		return 0;
	}
