...
public final class Castle extends AbstractResidence
{
	...
	public L2ResidenceTeleportZone getTeleZone()
	{
		if (_teleZone == null)
		{
			for (L2ResidenceTeleportZone zone : ZoneManager.getInstance().getAllZones(L2ResidenceTeleportZone.class))
			{
				...
				if (!getSiege().isInProgress()) // end siege
				{
					TerritoryWarManager.getInstance().getTerritory(getResidenceId()).setLordId(0);
					TerritoryWarManager.getInstance().getTerritory(getResidenceId()).updateDataInDB();
				}
			}
		}
		return _teleZone;
	...
	public void setOwner(L2Clan clan)
	{
		...
		if (!getSiege().isInProgress())
		{
			TerritoryWarManager.getInstance().getTerritory(getResidenceId()).updateState();
		}
	...
	public void removeOwner(L2Clan clan)
	{
		...
		// vGodFather
		TerritoryWarManager.getInstance().getTerritory(getResidenceId()).setOwnerClan(null);
		TerritoryWarManager.getInstance().getTerritory(getResidenceId()).setLordId(0);
		TerritoryWarManager.getInstance().getTerritory(getResidenceId()).updateDataInDB();
		TerritoryWarManager.getInstance().getTerritory(getResidenceId()).updateState();
