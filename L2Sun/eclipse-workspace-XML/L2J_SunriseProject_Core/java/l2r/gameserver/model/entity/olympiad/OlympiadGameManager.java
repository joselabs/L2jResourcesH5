...
public class OlympiadGameManager implements Runnable
{
	...
	public final void notifyCompetitorDamage(L2Character player, L2Character target, int damage)
	{
		...
		final L2PcInstance targetpc = target.getActingPlayer();
		
		if (targetpc.isInvul() || targetpc.isHpBlocked())
		{
			return;
		}
		
		if (!player.getActingPlayer().isInOlympiadMode() || !targetpc.isInOlympiadMode())
		{
			return;
		}
		
		if (targetpc.getOlympiadGameId() != player.getActingPlayer().getOlympiadGameId())
		{
			return;
		}
		
		final int id = player.getActingPlayer().getOlympiadGameId();
		if ((id < 0) || (id >= _tasks.length))
		{
			return;
		}
		...
		if (game != null)
		{
			game.addDamage(player.getActingPlayer(), damage);
		}
	}
