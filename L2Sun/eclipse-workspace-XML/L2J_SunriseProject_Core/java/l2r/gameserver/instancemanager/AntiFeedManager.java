...
public final class AntiFeedManager
{
	...
	{
		if (!AntiFeedConfigs.ANTIFEED_ENABLE)
		{
			return true;
		}
		...
		if ((AntiFeedConfigs.ANTIFEED_INTERVAL > 0) && _lastDeathTimes.containsKey(targetPlayer.getObjectId()))
		{
			if ((System.currentTimeMillis() - _lastDeathTimes.get(targetPlayer.getObjectId())) < AntiFeedConfigs.ANTIFEED_INTERVAL)
			{
				return false;
			}
		}
		
		if (AntiFeedConfigs.ANTIFEED_DUALBOX && (attacker != null))
		{
			...
			if ((targetClient == null) || (attackerClient == null) || targetClient.isDetached() || attackerClient.isDetached())
			{
				...
				return !AntiFeedConfigs.ANTIFEED_DISCONNECTED_AS_DUALBOX;
			}
	...
	public final boolean tryAddClient(int eventId, L2GameClient client, int max)
	{
		...
		int whiteListCount = DualboxChecksConfigs.DUALBOX_CHECK_WHITELIST.getOrDefault(addrHash, 0);
	...
	public final int getLimit(L2GameClient client, int max)
	{
		...
		final Integer addrHash = Integer.valueOf(client.getConnectionAddress().hashCode());
		int limit = max;
		if (DualboxChecksConfigs.DUALBOX_CHECK_WHITELIST.containsKey(addrHash))
		{
			limit += DualboxChecksConfigs.DUALBOX_CHECK_WHITELIST.get(addrHash);
		}
		return limit;
	}
