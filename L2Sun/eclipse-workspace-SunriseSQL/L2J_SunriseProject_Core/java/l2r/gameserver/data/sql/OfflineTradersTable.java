			...
			for (L2PcInstance pc : L2World.getInstance().getPlayers())
			{
				try
				{
					if (pc.isInStoreMode() && pc.isInOfflineMode())
					{
						...
						{
							case BUY:
								if (!OfflineTradeConfigs.OFFLINE_TRADE_ENABLE)
								{
									continue;
								}
							...
							case PACKAGE_SELL:
								...
								break;
							case MANUFACTURE:
								if (!OfflineTradeConfigs.OFFLINE_CRAFT_ENABLE)
								{
									continue;
								}
	...
	public void restoreOfflineTraders()
	{
			...
			ResultSet rs = stm.executeQuery(LOAD_OFFLINE_STATUS))
		{
			while (rs.next())
			{
				...
				if (OfflineTradeConfigs.OFFLINE_MAX_DAYS > 0)
				{
					Calendar cal = Calendar.getInstance();
					cal.setTimeInMillis(time);
					cal.add(Calendar.DAY_OF_YEAR, OfflineTradeConfigs.OFFLINE_MAX_DAYS);
				...
				try
				{
					...
					{
						player.getAppearance().setNameColor(OfflineTradeConfigs.OFFLINE_NAME_COLOR);
					}
