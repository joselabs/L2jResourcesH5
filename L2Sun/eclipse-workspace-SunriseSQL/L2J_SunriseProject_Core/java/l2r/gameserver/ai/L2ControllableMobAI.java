...
public final class L2ControllableMobAI extends L2AttackableAI
{
	...
	protected void thinkCast()
	{
		...
		if ((getInteractionTarget() == null) || getInteractionTarget().isAlikeDead())
		{
			setInteractionTarget(findNextRndTarget());
			npc.stopMove();
		}
		...
		npc.setTarget(getInteractionTarget());
		...
		if (!_actor.isMuted())
		{
			...
			{
				if (Util.checkIfInRange(sk.getCastRange(), _actor, getInteractionTarget(), true) && !_actor.isSkillDisabled(sk) && (_actor.getCurrentMp() > _actor.getStat().getMpConsume(sk)))
				{
					_actor.doCast(sk);
					return;
				}
			...
			if (!isNotMoving())
			{
				moveToPawn(getInteractionTarget(), max_range);
			}
	...
	protected void thinkAttack()
	{
		if ((getInteractionTarget() == null) || getInteractionTarget().isAlikeDead())
		{
			if (getInteractionTarget() != null)
			{
				// stop hating
				L2Attackable npc = (L2Attackable) _actor;
				npc.stopHating(getInteractionTarget());
			}
		...
		else
		{
			...
			if (((L2Npc) _actor).getClans() != null)
			{
				...
				for (L2Object obj : objs)
				{
					...
					if (_actor.isInsideRadius(npc, npc.getFactionRange(), false, true) && (Math.abs(getInteractionTarget().getZ() - npc.getZ()) < 200))
					{
						npc.getAI().notifyEvent(CtrlEvent.EVT_AGGRESSION, getInteractionTarget(), 1);
					}
				}
			}
			
			_actor.setTarget(getInteractionTarget());
			double dist2 = _actor.getPlanDistanceSq(getInteractionTarget().getX(), getInteractionTarget().getY());
			int range = _actor.getPhysicalAttackRange() + _actor.getTemplate().getCollisionRadius() + getInteractionTarget().getTemplate().getCollisionRadius();
			int max_range = range;
			...
			if (!_actor.isMuted() && (dist2 > ((range + 20) * (range + 20))))
			{
				...
				moveToPawn(getInteractionTarget(), range);
				return;
			}
			...
			if (hated != getInteractionTarget())
			{
				setInteractionTarget(hated);
			}
