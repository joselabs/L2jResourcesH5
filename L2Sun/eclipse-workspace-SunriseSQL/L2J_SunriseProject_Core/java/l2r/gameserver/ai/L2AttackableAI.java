...
public class L2AttackableAI extends L2CharacterAI implements Runnable
{
	...
	private final AtomicBoolean _thinking = new AtomicBoolean(false);
	...
	{
		if (checkTargetLost(getInteractionTarget()))
		{
			setInteractionTarget(null);
			return;
		}
		if (maybeMoveToPawn(getInteractionTarget(), _actor.getMagicalAttackRange(_skill)))
		{
			return;
		}
		clientStopMoving();
	...
	protected void thinkActive()
	{
		...
		if (_globalAggro >= 0)
		{
			...
			if (npc.isConfused())
			{
				hated = getInteractionTarget(); // effect handles selection
			}
			else
			{
	...
	protected void thinkAttack()
	{
		...
		final L2Character originalAttackTarget = getInteractionTarget();
		...
		if (tryMoveAroundOrDodge(npc, mostHate))
		{
			return;
		}
		...
		if ((dist2 > range) || !GeoData.getInstance().canSeeTarget(npc, mostHate))
		{
			...
			{
				final L2Character target = getInteractionTarget();
				if (target != null)
				{
		...
		// Attacks target
		_actor.doAttack(getInteractionTarget());
	}
	...
	protected boolean cast(L2Skill sk)
	{
		...
		if (getInteractionTarget() == null)
		{
			...
			{
				setInteractionTarget(caster.getMostHated());
			}
		}
		L2Character attackTarget = getInteractionTarget();
	...
	private L2Character effectTargetReconsider(L2Skill sk, boolean positive)
	{
		...
		if (!sk.hasEffectType(L2EffectType.CANCEL, L2EffectType.DISPEL, L2EffectType.DISPEL_BY_SLOT))
		{
			if (!positive)
			{
				...
				{
					if ((obj == null) || obj.isDead() || !GeoData.getInstance().canSeeTarget(actor, obj) || (obj == getInteractionTarget()))
					{
						continue;
					}
					try
					{
						actor.setTarget(getInteractionTarget());
					...
					if (dist2 <= range)
					{
						if (getInteractionTarget().getFirstEffect(sk) == null)
						{
							return obj;
						}
					}
		...
		else
		{
			...
			double dist2 = 0;
			int range = 0;
			range = sk.getCastRange() + actor.getTemplate().getCollisionRadius() + getInteractionTarget().getTemplate().getCollisionRadius();
			for (L2Character obj : actor.getKnownList().getKnownCharactersInRadius(range))
			{
				...
				if (obj instanceof L2Attackable)
				{
					if ((actor.getEnemyClan() != null) && actor.getEnemyClan().equals(((L2Attackable) obj).getClan()))
					{
						if (dist2 <= range)
						{
							if (getInteractionTarget().getFirstEffect(L2EffectType.BUFF) != null)
							{
								return obj;
							}
						}
	...
	protected void targetReconsider()
	{
		...
		if (!(actor instanceof L2GuardInstance))
		{
			...
			for (L2Object target : objs)
			{
				...
				if ((obj == null) || !GeoData.getInstance().canSeeTarget(actor, obj) || obj.isDead() || (obj != MostHate) || (obj == actor) || (obj == getInteractionTarget()))
				{
					continue;
				}
				if (obj.isPlayer())
	...
	protected void aggroReconsider()
	{
		...
		if (actor.getHateList() != null)
		{
			...
			for (L2Character obj : actor.getHateList())
			{
				...
				if ((obj == null) || !GeoData.getInstance().canSeeTarget(actor, obj) || obj.isDead() || (obj == getInteractionTarget()) || (obj == actor))
				{
					continue;
				}
	...
	protected void onEvtAttacked(L2Character attacker)
	{
		...
		else if (me.getMostHated() != getInteractionTarget())
		{
			setIntention(CtrlIntention.AI_INTENTION_ATTACK, attacker);
		}
	...
	private void factionCall(L2Attackable me, L2Character target)
	{
		...
		{
			final int factionRange = me.getClanHelpRange() + me.getTemplate().getCollisionRadius();
			...
			for (L2Object obj : me.getKnownList().getKnownCharactersInRadius(factionRange))
			{
				...
				if ((Math.abs(target.getZ() - called.getZ()) < 600) && me.getAttackByList().contains(target) && ((calledIntention == CtrlIntention.AI_INTENTION_IDLE) || (calledIntention == CtrlIntention.AI_INTENTION_ACTIVE)) && (called.getInstanceId() == me.getInstanceId()) && GeoData.getInstance().canSeeTarget(me, called))
				{
					if (target.isPlayable())
					{
						...
						{
							((L2Attackable) called).addDamageHate(getInteractionTarget(), 0, me.getHating(getInteractionTarget()));
					...
					else if ((called instanceof L2Attackable) && (getInteractionTarget() != null) && (calledIntention != CtrlIntention.AI_INTENTION_ATTACK))
					{
						((L2Attackable) called).addDamageHate(getInteractionTarget(), 0, me.getHating(getInteractionTarget()));
						called.getAI().setIntention(CtrlIntention.AI_INTENTION_ATTACK, getInteractionTarget());
					}
				}
			}
		}
