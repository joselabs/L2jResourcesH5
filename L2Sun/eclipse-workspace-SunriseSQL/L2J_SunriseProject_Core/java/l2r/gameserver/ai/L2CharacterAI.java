...
public class L2CharacterAI extends AbstractAI
{
	...
	protected boolean tryMoveAroundOrDodge(L2Character actor, L2Character mostHate)
	{
		if (!actor.isMovementDisabled())
		{
			final int range = actor.getPhysicalAttackRange();
			final int mostHateX = getInteractionTarget().getX();
			final int mostHateY = getInteractionTarget().getY();
			
			if (!actor.isInsideRadius(mostHateX, mostHateY, mostHate.getZ(), range, false, false))
			{
				final double angle = Math.toRadians(Rnd.get(-360, 360));
				final int targetX = mostHateX + (int) (range * Math.cos(angle));
				final int targetY = mostHateY + (int) (range * Math.sin(angle));
				moveTo(targetX, targetY, mostHate.getZ());
				return true;
			}
			
			if (actor.isNpc())
			{
				final L2Npc npc = (L2Npc) actor;
	...
	protected void onIntentionAttack(L2Character target)
	{
		...
		if (getIntention() == AI_INTENTION_ATTACK)
		{
			...
			if (getInteractionTarget() != target)
			{
	...
	protected void changeIntentionToCast(L2Skill skill, L2Object target)
	{
		...
		setInteractionTarget((L2Character) target);
	...
	protected void onEvtArrived()
	{
		...
		{
			// Cast target can be null if disconnected abnormally or similar
			if ((getInteractionTarget() != null) && !GeoData.getInstance().canMove(_actor, getInteractionTarget()))
			{
	...
	protected void onEvtForgetObject(L2Object object)
	{
		...
		if (getInteractionTarget() == object)
		{
	...
	public boolean canAura(L2Skill sk)
	{
		if ((sk.getTargetType() == L2TargetType.AURA) || (sk.getTargetType() == L2TargetType.BEHIND_AURA) || (sk.getTargetType() == L2TargetType.FRONT_AURA) || (sk.getTargetType() == L2TargetType.AURA_CORPSE_MOB) || (sk.getTargetType() == L2TargetType.AURA_UNDEAD_ENEMY))
		{
			...
			{
				if (target == getInteractionTarget())
				{
					return true;
				}
			}
	...
	public boolean canAOE(L2Skill sk)
	{
		if (sk.hasEffectType(L2EffectType.CANCEL, L2EffectType.DISPEL, L2EffectType.DISPEL_BY_SLOT))
		{
			if ((sk.getTargetType() == L2TargetType.AURA) || (sk.getTargetType() == L2TargetType.BEHIND_AURA) || (sk.getTargetType() == L2TargetType.FRONT_AURA) || (sk.getTargetType() == L2TargetType.AURA_CORPSE_MOB) || (sk.getTargetType() == L2TargetType.AURA_UNDEAD_ENEMY))
			{
				...
				for (L2Character target : _actor.getKnownList().getKnownCharactersInRadius(sk.getAffectRange()))
				{
					...
					if (target instanceof L2Attackable)
					{
						...
						if ((targets.getEnemyClan() == null) || (actors.getClan() == null) || !targets.getEnemyClan().equals(actors.getClan()) || ((actors.getClan() == null) && (!actors.isChaos())))
						{
							continue;
						}
					}
			...
			{
				boolean cancast = true;
				for (L2Character target : getInteractionTarget().getKnownList().getKnownCharactersInRadius(sk.getAffectRange()))
				{
