...
public abstract class L2Object extends ListenersContainer implements IIdentifiable, INamable, IUniqueId, IPositionable
{
	...
	public void setGeoIndex(int index)
	{
		_geoIndex = index;
	}
	
	// vGodFather packet system controlling packets
	public Future<?> _updateAndBroadcastStatus;
	public Future<?> _updateStatus;
	public Future<?> _effectsUpdateTask;
	public Future<?> _userInfoTask;
	public Future<?> _charInfoTask;
	public Future<?> _npcInfoTask;
	...
	public void decayMe()
	{
		...
		if (isAttackable())
		{
			L2World.getInstance().getPlayers().stream().forEach(pc -> pc.getKnownList().removeKnownObject(this));
		}
	}
	...
	{
		// vGodFather
		if ((_npcInfoTask != null) && !_npcInfoTask.isDone())
		{
			return;
		}
