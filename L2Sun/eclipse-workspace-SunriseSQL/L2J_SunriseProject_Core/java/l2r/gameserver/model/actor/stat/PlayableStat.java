...
public class PlayableStat extends CharStat
{
	...
	public boolean addExp(long value)
	{
		...
		for (int tmp = currentLevel; tmp <= maxLevel; tmp++)
		{
			if (currentExp >= getExpForLevel(tmp))
			{
				...
				if (tmp != currentLevel)
				{
					final int newLevel = tmp - currentLevel;
					
					addLevel(newLevel, value);
				}
				break;
			}
		}
	...
	public boolean addLevel(int value, long exp)
	{
		...
		// int tmpLvl = getActiveChar().isPlayer() && !getActiveChar().isGM() ? getLevel() : -1;
