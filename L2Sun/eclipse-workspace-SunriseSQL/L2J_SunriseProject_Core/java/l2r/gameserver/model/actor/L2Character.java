...
import l2r.gameserver.MoveController;
...
public abstract class L2Character extends L2Object implements ISkillsHolder
{
	...
	public void teleToLocation(int x, int y, int z, int heading, int instanceId, int randomOffset)
	{
		...
		// vGodFather addon
		Location fixedLoc = GeoData.getInstance().moveCheck(originalX, originalY, z, x, y, z, getGeoIndex());
		x = fixedLoc.getX();
		y = fixedLoc.getY();
	...
	private boolean doAttackHitByPole(Attack attack, L2Character target, int sAtk)
	{
		...
		if (_fanRange != null)
		{
			...
			for (L2Character obj : objs)
			{
				...
				if (isAttackable() && obj.isAttackable() && (((L2Attackable) this).getEnemyClan() == null) && (!((L2Attackable) this).isChaos()))
				{
					continue;
				}
				
				if (isAttackable() && obj.isAttackable() && !((L2Attackable) this).getEnemyClan().equals(((L2Attackable) obj).getClan()) && (!((L2Attackable) this).isChaos()))
				{
					continue;
				}
				...
				if (!obj.isAlikeDead())
				{
					if ((obj == getAI().getInteractionTarget()) || obj.isAutoAttackable(this))
					{
	...
	private void beginCast(L2Skill skill, boolean simultaneously, L2Character target, L2Object[] targets)
	{
		...
		// Skill reuse check
		if ((reuseDelay > 2000) && !skillMastery)
		{
		...
		if (!skill.isToggle())
		{
			...
			{
				case ARTILLERY:
					broadcastPacket(new MagicSkillUse(getActingPlayer(), target, skill.getDisplayId(), skill.getDisplayLevel(), hitTime, reuseDelay));
					break;
				default:
	 ...
	 */
	public void stopAllEffects(boolean all)
	{
		_effects.stopAllEffects(all);
	}
	...
	{
		return hasAI() && ((getAI().getInteractionTarget() != null) || getAI().isAutoAttacking());
	}
	...
	public void onHitTimer(L2Character target, int damage, boolean crit, boolean miss, boolean soulshot, byte shld)
	{
		...
		if (!miss && (damage > 0))
		{
			...
			if (reflectedDamage > 0)
			{
				...
				OlympiadGameManager.getInstance().notifyCompetitorDamage(target, this, reflectedDamage);
			}
	...
	public void onMagicLaunchedTimer(MagicUseTask mut)
	{
		...
		L2Object[] targets = isPlayer() ? skill.isAura() ? skill.getTargetList(this) : skill.isArea() ? skill.getTargetList(this, false, getAI().getInteractionTarget()) : mut.getTargets() : mut.getTargets();
	...
	public void onMagicHitTimer(MagicUseTask mut)
	{
		...
		for (L2Object tgt : targets)
		{
			if (tgt.isPlayable())
			{
				...
				if ((skill.getSkillType() == L2SkillType.BUFF) && !target.isBuffBlocked())
				{
	...
	public void callSkill(L2Skill skill, L2Object[] targets)
	{
		try
		{
			...
			for (L2Object obj : targets)
			{
				...
				if (target.hasAI())
				{
					targetsAttackTarget = target.getAI().getInteractionTarget();
					targetsCastTarget = target.getAI().getInteractionTarget();
				}
	...
	protected boolean canAttackSiegier(L2Character activeChar, L2Object target)
	{
		...
		if (activeChar.isPlayable() && activeChar.isInsideZone(ZoneIdType.SIEGE))
		{
			...
			if (hall != null)
			{
				...
				{
					if (player.isInSameParty(target.getActingPlayer()) || player.isInSameChannel(target.getActingPlayer()) || player.isInSameClan(target.getActingPlayer()) || player.isInSameAlly(target.getActingPlayer()))
					{
						return false;
					}
			...
			final Siege siege = SiegeManager.getInstance().getSiege(target);
		...
		if ((target.getActingPlayer() != null) && (activeChar.getSiegeState() > 0) && activeChar.isInsideZone(ZoneIdType.SIEGE) && (target.getActingPlayer().getSiegeState() == activeChar.getSiegeState()) && (target.getActingPlayer() != activeChar) && (target.getActingPlayer().getSiegeSide() == activeChar.getSiegeSide()))
		{
			...
			if (fort == null)
			{
				...
				if (siege != null)
				{
					if (siege.isAttackersInAlly() || getActingPlayer().isInSameParty(target.getActingPlayer()) || getActingPlayer().isInSameChannel(target.getActingPlayer()) || getActingPlayer().isInSameClan(target.getActingPlayer()) || getActingPlayer().isInSameAlly(target.getActingPlayer()))
					{
						sendPacket(SystemMessageId.FORCED_ATTACK_IS_IMPOSSIBLE_AGAINST_SIEGE_SIDE_TEMPORARY_ALLIED_MEMBERS);
						sendPacket(ActionFailed.STATIC_PACKET);
						return false;
	...
	public int getWaterZ()
	{
		...
		for (final L2ZoneType zone : ZoneManager.getInstance().getZones(getX(), getY(), getZ()))
		{
			if (zone instanceof L2WaterZone)
			{
				waterZone = (L2WaterZone) zone;
				return waterZone.getWaterZ() + 16;
			}
		}
