...
public class CharEffectList
{
	...
	public final void stopAllEffects(boolean all)
	{
		if (all)
		{
			...
			getBuffs().stream().filter(e -> e != null).forEach(e -> stopSkillEffects(e.getSkill().getId()));
			getDebuffs().stream().filter(e -> e != null).forEach(e -> stopSkillEffects(e.getSkill().getId()));
		}
		else
		{
			getEffects().stream().filter(e -> (e != null) && e.canBeStolen()).forEach(e -> e.exit(true));
			
			getBuffs().stream().filter(e -> (e != null) && e.canBeStolen()).forEach(e -> stopSkillEffects(e.getSkill().getId()));
			getDebuffs().stream().filter(e -> (e != null) && e.canBeStolen()).forEach(e -> stopSkillEffects(e.getSkill().getId()));
		}
	}
